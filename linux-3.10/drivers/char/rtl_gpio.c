...
static unsigned int wps_btn_gnum, wps_led_gnum, reset_btn_gnum, reset_led_gnum, rtl819x_btn_actlow = 0;
static unsigned int sta_r_led_gnum, sta_g_led_gnum, sta_b_led_gnum, rtl819x_led_actlow = 0;

int sta_led_color = 0;    //the color of sta led, 0-red, 1-green, 2-bule
int sta_led_mode = 0;     //the mode of sta led, 0-off, 1-on, 2-blink
...
void sta_led_on(void)
{
    if(sta_led_color == 0)
    {
        gpio_set_value(sta_r_led_gnum, (rtl819x_led_actlow ? 0 : 1));
        gpio_set_value(sta_g_led_gnum, (rtl819x_led_actlow ? 1 : 0));
        gpio_set_value(sta_b_led_gnum, (rtl819x_led_actlow ? 1 : 0));
    }
    else if(sta_led_color == 1)
    {
        gpio_set_value(sta_r_led_gnum, (rtl819x_led_actlow ? 1 : 0));
        gpio_set_value(sta_g_led_gnum, (rtl819x_led_actlow ? 0 : 1));
        gpio_set_value(sta_b_led_gnum, (rtl819x_led_actlow ? 1 : 0));
    }
    else if(sta_led_color == 2)
    {
        gpio_set_value(sta_r_led_gnum, (rtl819x_led_actlow ? 1 : 0));
        gpio_set_value(sta_g_led_gnum, (rtl819x_led_actlow ? 1 : 0));
        gpio_set_value(sta_b_led_gnum, (rtl819x_led_actlow ? 0 : 1));
    }
     else if(sta_led_color == 3)
    {
        gpio_set_value(sta_r_led_gnum, (rtl819x_led_actlow ? 0 : 1));
        gpio_set_value(sta_g_led_gnum, (rtl819x_led_actlow ? 0 : 1));
        gpio_set_value(sta_b_led_gnum, (rtl819x_led_actlow ? 1 : 0));
    }
      else if(sta_led_color == 4)
    {
        gpio_set_value(sta_r_led_gnum, (rtl819x_led_actlow ? 0 : 1));
        gpio_set_value(sta_g_led_gnum, (rtl819x_led_actlow ? 1 : 0));
        gpio_set_value(sta_b_led_gnum, (rtl819x_led_actlow ? 0 : 1));
...
bool is_sta_led_on(void)
{
    if( (gpio_get_value(sta_r_led_gnum) ^ (rtl819x_led_actlow ? 1 : 0 ))
        || (gpio_get_value(sta_g_led_gnum) ^ (rtl819x_led_actlow ? 1 : 0 ))
        || (gpio_get_value(sta_b_led_gnum) ^ (rtl819x_led_actlow ? 1 : 0 )) )
    {
        return true;
    }
    else
...
{	
	...
	if(rtl819x_bond_option() == BSP_BOND_97FN) {
		...
		DPRINTK("line: %d\n", __LINE__);
		return !RTLWIFINIC_GPIO_read(reset_btn_gnum) ^ (rtl819x_btn_actlow ? 1 : 0 );
#endif
	}
#endif

	return gpio_get_value(reset_btn_gnum) ^ (rtl819x_btn_actlow ? 1 : 0 );
}
...
void reset_led_on(void)
{
...
#if !defined(CONFIG_FREE_WBB_PIN)	
		DPRINTK("line: %d\n", __LINE__);
		RTLWIFINIC_GPIO_write(reset_led_gnum, (rtl819x_led_actlow ? 0 : 1));
		return;
#endif
	}
#endif
	...
	RTLWIFINIC_GPIO_write(reset_led_gnum, (rtl819x_led_actlow ? 0 : 1));
#else
	//gpio_set_value(reset_led_gnum, (rtl819x_led_actlow ? 0 : 1));
	//sta_led_on();
	gpio_set_value(sta_r_led_gnum, (rtl819x_led_actlow ? 0 : 1));
    gpio_set_value(sta_g_led_gnum, (rtl819x_led_actlow ? 1 : 0));
    gpio_set_value(sta_b_led_gnum, (rtl819x_led_actlow ? 1 : 0));
...
void reset_led_off(void)
{
...
#if !defined(CONFIG_FREE_WBB_PIN)	
		DPRINTK("line: %d\n", __LINE__);
		RTLWIFINIC_GPIO_write(reset_led_gnum, (rtl819x_led_actlow ? 1 : 0));
		return;
#endif
	}
#endif
	...
	RTLWIFINIC_GPIO_write(reset_led_gnum, (rtl819x_led_actlow ? 1 : 0));
#else
	//gpio_set_value(reset_led_gnum, (rtl819x_led_actlow ? 1 : 0));
	sta_led_off();
#endif
}
...
{
	//gpio_set_value(wps_led_gnum, (rtl819x_led_actlow ? 1 : 0));
	sta_led_off();
	AutoCfg_LED_Blink = 0;
    sta_led_mode = 0;
}
...
{
	//gpio_set_value(wps_led_gnum, (rtl819x_led_actlow ? 0 : 1));
	sta_led_on();
	AutoCfg_LED_Blink = 0;
    sta_led_mode = 1;
}
...
{
	return gpio_get_value(wps_btn_gnum) ^ (rtl819x_btn_actlow ? 1 : 0);
}
...
//				DPRINTK("2-5 reset 2\n");
    ...
    if(probe_state == PROBE_ACTIVE && probe_counter >= PROBE_TIME)
    {
        mod_timer(&probe_timer, jiffies + HZ/4);
    }
    else
    {
#ifdef AUTO_CONFIG
...
static int read_sta_led_color(struct seq_file *s, void *v)
{
    seq_printf(s, "%d\n", sta_led_color);
	return 0;
}

static int write_sta_led_color(struct file *file, const char *buffer,
				unsigned long count, void *data)
{
	char flag[20];
    int value;
	...
	if (buffer && !copy_from_user(&flag, buffer, 1)) {
        value = simple_strtol(flag, NULL, 10);
        if(value >= 0 && value <= 4)
        {
		    sta_led_color = value;
            if(is_sta_led_on())
...
int sta_led_color_single_open(struct inode *inode, struct file *file)
{
        return(single_open(file, read_sta_led_color, NULL));
}

static ssize_t sta_led_color_single_write(struct file * file, const char __user * userbuf,
		     size_t count, loff_t * off)
{
	return write_sta_led_color(file, userbuf,count, off);
}

struct file_operations sta_led_color_fops = {
        .open           = sta_led_color_single_open,
	    .write           = sta_led_color_single_write,
        .read            = seq_read,
        .llseek          = seq_lseek,
        .release        = single_release,
};

static int read_sta_led_mode(struct seq_file *s, void *v)
{
    seq_printf(s, "%d\n", sta_led_mode);
	return 0;
}

static int write_sta_led_mode(struct file *file, const char *buffer,
				unsigned long count, void *data)
{
	char flag[20];
    int value;
...
int sta_led_mode_single_open(struct inode *inode, struct file *file)
{
        return(single_open(file, read_sta_led_mode, NULL));
}

static ssize_t sta_led_mode_single_write(struct file * file, const char __user * userbuf,
		     size_t count, loff_t * off)
{
	return write_sta_led_mode(file, userbuf,count, off);
}

struct file_operations sta_led_mode_fops = {
        .open           = sta_led_mode_single_open,
	    .write           = sta_led_mode_single_write,
        .read            = seq_read,
        .llseek          = seq_lseek,
        .release        = single_release,
};
...
int __init rtl_gpio_init(void)
{
    ...
    proc_create_data("gpio_color", 0, &proc_root,
			 &sta_led_color_fops, NULL);
    DPRINTK("create gpio_color OK!!!\n");
    proc_create_data("gpio_mode", 0, &proc_root,
			 &sta_led_mode_fops, NULL);
    DPRINTK("create gpio_mode OK!!!\n");
...
static int rtl819x_led_probe(struct platform_device *dev) {
    ...
    DPRINTK("probe led device = %s\n",pdata->name);
    
	if(!strcmp(pdata->name, "reset led"))
		reset_led_gnum = pdata->gpio;
	else if(!strcmp(pdata->name, "wps led"))
		wps_led_gnum = pdata->gpio;
    else if(!strcmp(pdata->name, "sta_r led"))
		sta_r_led_gnum = pdata->gpio;
    else if(!strcmp(pdata->name, "sta_g led"))
		sta_g_led_gnum = pdata->gpio;
    else if(!strcmp(pdata->name, "sta_b led"))
		sta_b_led_gnum = pdata->gpio;
...
OUT:	
	...
	if(pdata->flags & RTL819X_GPIO_ACTLOW) {
		gpio_sysfs_set_active_low(pdata->gpio, 1);
        rtl819x_btn_actlow  = RTL819X_GPIO_ACTLOW;
	}
