...
void odm_config_rf_radio_a_8812f(struct dm_struct *dm, u32 addr, u32 data)
{
	...
	odm_config_rf_reg_8812f(dm, addr, data, RF_PATH_A, addr |
				maskfor_phy_set);
...
void odm_config_rf_radio_b_8812f(struct dm_struct *dm, u32 addr, u32 data)
{
	...
	odm_config_rf_reg_8812f(dm, addr, data, RF_PATH_B, addr |
				maskfor_phy_set);
...
void odm_update_agc_big_jump_lmt_8812f(struct dm_struct *dm, u32 addr, u32 data)
{
#if 0
	...
	dbg_print("data = 0x%x, rf_gain_idx = 0x%x, bb_gain_idx = 0x%x,
		  agc_table_idx = 0x%x\n", data, rf_gain_idx, bb_gain_idx,
		  agc_table_idx);
...
void odm_config_bb_agc_8812f(struct dm_struct *dm, u32 addr, u32 bitmask,
			     u32 data)
{
	/* odm_update_agc_big_jump_lmt_8812f(dm, addr, data); */
