	...
	void *dm_void)
{
	...
	u32 thermal_offset_tmp = 0, thermal_offset_index = 0x10, thermal_tmp = 0xff;
	...
	RF_DBG(dm, DBG_RF_TX_PWR_TRACK,
	       "===>%s channel=%d thermal pahtA=0x%x cali_info->txpowertrack_control=%d\n",
	       __func__, channel, thermal, cali_info->txpowertrack_control);

	if (thermal == 0xff || cali_info->txpowertrack_control == 4) {
		i = 0;
		while (i < 64) {
