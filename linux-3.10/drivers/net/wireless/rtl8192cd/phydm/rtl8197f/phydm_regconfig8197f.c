	...
	u32 data)
{
	...
	odm_config_rf_reg_8197f(dm, addr, data, RF_PATH_A, addr | maskfor_phy_set);
	...
	u32 data)
{
	...
	odm_config_rf_reg_8197f(dm, addr, data, RF_PATH_B, addr | maskfor_phy_set);
	...
	u32 data)
{
	...
	if (bb_gain_idx > 0x3c) {
		if (rf_gain_idx == dig_tab->rf_gain_idx && is_limit == false) {
			is_limit = true;
			dig_tab->big_jump_lmt[big_jump_lmt_idx] = bb_gain_idx - 2;
			PHYDM_DBG(dm, DBG_DIG,
				  "===> [AGC_TAB] big_jump_lmt [%d] = 0x%x\n",
				  big_jump_lmt_idx,
				  dig_tab->big_jump_lmt[big_jump_lmt_idx]);
		}
	} else
		is_limit = false;
...
void odm_config_bb_agc_8197f(
	...
	u32 data)
{
	odm_update_agc_big_jump_lmt_8197f(dm, addr, data);
