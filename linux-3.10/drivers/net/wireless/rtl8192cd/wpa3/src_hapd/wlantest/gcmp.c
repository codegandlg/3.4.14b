...
void gcmp_aad_nonce(const struct ieee80211_hdr *hdr, const u8 * data, u8 * aad, size_t * aad_len, u8 * nonce)
{
	u16 fc, stype, seq;
	int qos = 0, addr4 = 0;
	u8 *pos;
		 ...
		 const u8 * data, size_t data_len, size_t * decrypted_len)
{
	...
	if (aes_gcm_ad(tk, tk_len, nonce, sizeof(nonce), m, mlen, aad, aad_len, m + mlen, plain) < 0) {
...
u8 *gcmp_encrypt(const u8 * tk, size_t tk_len, const u8 * frame, size_t len,
		 size_t hdrlen, const u8 * qos, const u8 * pn, int keyid, size_t * encrypted_len)
{
	...
	*pos++ = pn[0];		/* PN5 */
	...
	if (aes_gcm_ae(tk, tk_len, nonce, sizeof(nonce), frame + hdrlen, plen, aad, aad_len, pos, pos + plen) < 0) {
		os_free(crypt);
		return NULL;
	}
