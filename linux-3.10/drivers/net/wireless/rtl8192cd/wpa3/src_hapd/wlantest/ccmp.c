...
void ccmp_aad_nonce(const struct ieee80211_hdr *hdr, const u8 * data, u8 * aad, size_t * aad_len, u8 * nonce)
{
	u16 fc, stype, seq;
	int qos = 0, addr4 = 0;
	u8 *pos;
		 ...
		 const u8 * data, size_t data_len, size_t * decrypted_len)
{
	...
	if (aes_ccm_ad(tk, 16, nonce, 8, data + 8, mlen, aad, aad_len, data + 8 + mlen, plain) < 0) {
		...
		wpa_printf(MSG_INFO, "Invalid CCMP MIC in frame: A1=" MACSTR
			   ...
			   MAC2STR(hdr->addr3), WLAN_GET_SEQ_SEQ(seq_ctrl), WLAN_GET_SEQ_FRAG(seq_ctrl));
		os_free(plain);
		return NULL;
	}
		 ...
		 u8 * pn, int keyid, size_t * encrypted_len)
{
	...
	*pos++ = pn[0];		/* PN5 */
	...
	if (aes_ccm_ae(tk, 16, nonce, 8, frame + hdrlen, plen, aad, aad_len, pos, pos + plen) < 0) {
		os_free(crypt);
		return NULL;
	}
		     ...
		     const u8 * data, size_t data_len, size_t * decrypted_len)
{
	...
	if (aes_ccm_ad(tk, 32, nonce, 16, data + 8, mlen, aad, aad_len, data + 8 + mlen, plain) < 0) {
		u16 seq_ctrl = le_to_host16(hdr->seq_ctrl);
		     ...
		     u8 * qos, u8 * pn, int keyid, size_t * encrypted_len)
{
	...
	*pos++ = pn[0];		/* PN5 */
	...
	if (aes_ccm_ae(tk, 32, nonce, 16, frame + hdrlen, plen, aad, aad_len, pos, pos + plen) < 0) {
		os_free(crypt);
		return NULL;
	}

	wpa_hexdump(MSG_EXCESSIVE, "CCMP-256 encrypted", crypt + hdrlen + 8, plen);
