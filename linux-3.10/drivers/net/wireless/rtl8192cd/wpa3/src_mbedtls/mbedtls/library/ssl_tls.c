...
int (*mbedtls_ssl_hw_record_init) (mbedtls_ssl_context * ssl,
				   ...
				   size_t ivlen,
				   const unsigned char *mac_enc, const unsigned char *mac_dec, size_t maclen) = NULL;
...
static int ssl3_prf(const unsigned char *secret, size_t slen,
		    const char *label, const unsigned char *random, size_t rlen, unsigned char *dstbuf, size_t dlen)
{
	size_t i;
...
int mbedtls_ssl_derive_keys(mbedtls_ssl_context * ssl)
{
	...
	if (handshake->resume == 0) {
		...
		if (ssl->handshake->extended_ms == MBEDTLS_SSL_EXTENDED_MS_ENABLED) {
			...
			ret = handshake->tls_prf(handshake->premaster, handshake->pmslen,
						 "extended master secret", session_hash, hash_len, session->master, 48);
			if (ret != 0) {
...
#endif
			ret = handshake->tls_prf(handshake->premaster, handshake->pmslen,
						 "master secret", handshake->randbytes, 64, session->master, 48);
		if (ret != 0) {
			MBEDTLS_SSL_DEBUG_RET(1, "prf", ret);
			return (ret);
		}
	...
	if (cipher_info->mode == MBEDTLS_MODE_GCM || cipher_info->mode == MBEDTLS_MODE_CCM) {
		...
		if (cipher_info->mode == MBEDTLS_MODE_STREAM)
			...
			{
				transform->minlen = transform->maclen
				    + cipher_info->block_size - transform->maclen % cipher_info->block_size;
			}
	...
	MBEDTLS_SSL_DEBUG_MSG(3, ("keylen: %d, minlen: %d, ivlen: %d, maclen: %d",
				  transform->keylen, transform->minlen, transform->ivlen, transform->maclen));
	...
	if (mbedtls_ssl_hw_record_init != NULL) {
		...
		if ((ret = mbedtls_ssl_hw_record_init(ssl, key1, key2, transform->keylen,
						      ...
						      iv_copy_len, mac_enc, mac_dec, transform->maclen)) != 0) {
	...
	if (ssl->conf->f_export_keys != NULL) {
		ssl->conf->f_export_keys(ssl->conf->p_export_keys,
					 session->master, keyblk, transform->maclen, transform->keylen, iv_copy_len);
	}
#endif
	...
	if (session->compression == MBEDTLS_SSL_COMPRESS_DEFLATE) {
		...
		if (deflateInit(&transform->ctx_deflate,
				Z_DEFAULT_COMPRESSION) != Z_OK || inflateInit(&transform->ctx_inflate) != Z_OK) {
...
int mbedtls_ssl_psk_derive_premaster(mbedtls_ssl_context * ssl, mbedtls_key_exchange_type_t key_ex)
{
	...
	if (key_ex == MBEDTLS_KEY_EXCHANGE_ECDHE_PSK) {
		...
		if ((ret = mbedtls_ecdh_calc_secret(&ssl->handshake->ecdh_ctx, &zlen,
						    p + 2, end - (p + 2), ssl->conf->f_rng, ssl->conf->p_rng)) != 0) {
...
static void ssl_mac(mbedtls_md_context_t * md_ctx, unsigned char *secret,
		    unsigned char *buf, size_t len, unsigned char *ctr, int type)
{
...
static int ssl_encrypt_buf(mbedtls_ssl_context * ssl)
{
	...
	if (mode == MBEDTLS_MODE_CBC) {
		...
		if ((ret = mbedtls_cipher_crypt(&ssl->transform_out->cipher_ctx_enc,
						ssl->transform_out->iv_enc,
						ssl->transform_out->ivlen, enc_msg, enc_msglen, enc_msg, &olen)) != 0) {
			MBEDTLS_SSL_DEBUG_RET(1, "mbedtls_cipher_crypt", ret);
			return (ret);
		}
		...
		if (ssl->minor_ver < MBEDTLS_SSL_MINOR_VERSION_2) {
			...
			memcpy(ssl->transform_out->iv_enc,
			       ssl->transform_out->cipher_ctx_enc.iv, ssl->transform_out->ivlen);
		}
#endif
...
static int ssl_decrypt_buf(mbedtls_ssl_context * ssl)
{
	...
	if (mode == MBEDTLS_MODE_STREAM) {
		...
		if ((ret = mbedtls_cipher_crypt(&ssl->transform_in->cipher_ctx_dec,
						...
						ssl->in_msg, ssl->in_msglen, ssl->in_msg, &olen)) != 0) {
			MBEDTLS_SSL_DEBUG_RET(1, "mbedtls_cipher_crypt", ret);
			return (ret);
		}
	...
	if (mode == MBEDTLS_MODE_GCM || mode == MBEDTLS_MODE_CCM) {
		...
		if (ssl->in_msglen < explicit_iv_len + taglen) {
			MBEDTLS_SSL_DEBUG_MSG(1, ("msglen (%d) < explicit_iv_len (%d) "
						  "+ taglen (%d)", ssl->in_msglen, explicit_iv_len, taglen));
			return (MBEDTLS_ERR_SSL_INVALID_MAC);
		}
		...
		memcpy(ssl->transform_in->iv_dec + ssl->transform_in->fixed_ivlen,
		       ssl->in_iv, ssl->transform_in->ivlen - ssl->transform_in->fixed_ivlen);
	...
	if (mode == MBEDTLS_MODE_CBC) {
		...
		if (ssl->in_msglen < minlen + ssl->transform_in->ivlen ||
			...
			MBEDTLS_SSL_DEBUG_MSG(1, ("msglen (%d) < max( ivlen(%d), maclen (%d) "
						  ...
						  ssl->transform_in->ivlen, ssl->transform_in->maclen));
			return (MBEDTLS_ERR_SSL_INVALID_MAC);
		}
		...
		if ((ret = mbedtls_cipher_crypt(&ssl->transform_in->cipher_ctx_dec,
						ssl->transform_in->iv_dec,
						ssl->transform_in->ivlen,
						dec_msg, dec_msglen, dec_msg_result, &olen)) != 0) {
			MBEDTLS_SSL_DEBUG_RET(1, "mbedtls_cipher_crypt", ret);
			return (ret);
		}
		...
		if (ssl->minor_ver < MBEDTLS_SSL_MINOR_VERSION_2) {
			...
			memcpy(ssl->transform_in->iv_dec,
			       ssl->transform_in->cipher_ctx_dec.iv, ssl->transform_in->ivlen);
		}
#endif
	...
	if (auth_done == 0) {
		...
		if (ssl->minor_ver == MBEDTLS_SSL_MINOR_VERSION_0) {
			ssl_mac(&ssl->transform_in->md_ctx_dec,
				ssl->transform_in->mac_dec, ssl->in_msg, ssl->in_msglen, ssl->in_ctr, ssl->in_msgtype);
		} else
#endif				/* MBEDTLS_SSL_PROTO_SSL3 */
#if defined(MBEDTLS_SSL_PROTO_TLS1) || defined(MBEDTLS_SSL_PROTO_TLS1_1) || \
        defined(MBEDTLS_SSL_PROTO_TLS1_2)
...
int mbedtls_ssl_write_record(mbedtls_ssl_context * ssl)
{
...
#if defined(MBEDTLS_SSL_PROTO_DTLS)
	if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM &&
	    ssl->handshake != NULL && ssl->handshake->retransmit_state == MBEDTLS_SSL_RETRANS_SENDING) {
	...
	if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM &&
	    ssl->handshake != NULL &&
	    ssl->handshake->retransmit_state != MBEDTLS_SSL_RETRANS_SENDING &&
	    (ssl->out_msgtype == MBEDTLS_SSL_MSG_CHANGE_CIPHER_SPEC || ssl->out_msgtype == MBEDTLS_SSL_MSG_HANDSHAKE)) {
...
static int ssl_prepare_handshake_record(mbedtls_ssl_context * ssl)
{
	...
	MBEDTLS_SSL_DEBUG_MSG(3, ("handshake message: msglen ="
				  " %d, type = %d, hslen = %d", ssl->in_msglen, ssl->in_msg[0], ssl->in_hslen));
...
static inline uint64_t ssl_load_six_bytes(unsigned char *buf)
{
	return (((uint64_t) buf[0] << 40) |
		((uint64_t) buf[1] << 32) |
		((uint64_t) buf[2] << 24) | ((uint64_t) buf[3] << 16) | ((uint64_t) buf[4] << 8) | ((uint64_t) buf[5]));
}
					...
					unsigned char *obuf, size_t buf_len, size_t * olen)
{
	...
	if (in_len < 61 ||
	    ...
	    in[3] != 0 || in[4] != 0 || in[19] != 0 || in[20] != 0 || in[21] != 0) {
...
static int ssl_handle_possible_reconnect(mbedtls_ssl_context * ssl)
{
	...
	ret = ssl_check_dtls_clihlo_cookie(ssl->conf->f_cookie_write,
					   ssl->conf->f_cookie_check,
					   ssl->conf->p_cookie,
					   ssl->cli_id, ssl->cli_id_len,
					   ssl->in_buf, ssl->in_left, ssl->out_buf, MBEDTLS_SSL_MAX_CONTENT_LEN, &len);
...
static int ssl_parse_record_header(mbedtls_ssl_context * ssl)
{
    ...
    defined(MBEDTLS_SSL_PROTO_TLS1_2)
		 ...
		 */
		if (ssl->minor_ver >= MBEDTLS_SSL_MINOR_VERSION_1 &&
		    ssl->in_msglen > ssl->transform_in->minlen + MBEDTLS_SSL_MAX_CONTENT_LEN + 256) {
			MBEDTLS_SSL_DEBUG_MSG(1, ("bad message length"));
			return (MBEDTLS_ERR_SSL_INVALID_RECORD);
		}
#endif
	...
	if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM) {
		...
		if (rec_epoch != ssl->in_epoch) {
			MBEDTLS_SSL_DEBUG_MSG(1, ("record from another epoch: "
						  "expected %d, received %d", ssl->in_epoch, rec_epoch));
...
int mbedtls_ssl_parse_certificate(mbedtls_ssl_context * ssl)
{
	...
	if (ssl->conf->endpoint == MBEDTLS_SSL_IS_CLIENT && ssl->renego_status == MBEDTLS_SSL_RENEGOTIATION_IN_PROGRESS) {
		...
		if (ssl->session->peer_cert->raw.len !=
		    ...
		    memcmp(ssl->session->peer_cert->raw.p,
			   ssl->session_negotiate->peer_cert->raw.p, ssl->session->peer_cert->raw.len) != 0) {
	...
	if (authmode != MBEDTLS_SSL_VERIFY_NONE) {
		 ...
		 */
		ret = mbedtls_x509_crt_verify_with_profile(ssl->session_negotiate->peer_cert,
							   ca_chain, ca_crl,
		...
		if (mbedtls_ssl_check_cert_usage(ssl->session_negotiate->peer_cert,
						 ciphersuite_info,
						 !ssl->conf->endpoint, &ssl->session_negotiate->verify_result) != 0) {
...
int mbedtls_ssl_set_session(mbedtls_ssl_context * ssl, const mbedtls_ssl_session * session)
{
	...
	if (ssl == NULL ||
	    session == NULL || ssl->session_negotiate == NULL || ssl->conf->endpoint != MBEDTLS_SSL_IS_CLIENT) {
		return (MBEDTLS_ERR_SSL_BAD_INPUT_DATA);
	}
...
int mbedtls_ssl_set_hs_ecjpake_password(mbedtls_ssl_context * ssl, const unsigned char *pw, size_t pw_len)
{
	...
	return (mbedtls_ecjpake_setup(&ssl->handshake->ecjpake_ctx,
				      role, MBEDTLS_MD_SHA256, MBEDTLS_ECP_DP_SECP256R1, pw, pw_len));
}
	...
	if (len > max_len) {
		...
		if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM) {
			MBEDTLS_SSL_DEBUG_MSG(1, ("fragment larger than the (negotiated) "
						  "maximum fragment length: %d > %d", len, max_len));
			return (MBEDTLS_ERR_SSL_BAD_INPUT_DATA);
		} else
#endif
			len = max_len;
...
int mbedtls_ssl_check_cert_usage(const mbedtls_x509_crt * cert,
				 const mbedtls_ssl_ciphersuite_t * ciphersuite, int cert_endpoint, uint32_t * flags)
{
	int ret = 0;
