...
int mbedtls_net_bind(mbedtls_net_context * ctx, const char *bind_ip, const char *port, int proto)
{
	...
	if (bind_ip != NULL) {
		...
		if (n == 4)
			server_addr.sin_addr.s_addr = net_htonl(((uint32_t) c[0] << 24) |
								((uint32_t) c[1] << 16) |
...
int mbedtls_net_accept(mbedtls_net_context * bind_ctx,
		       mbedtls_net_context * client_ctx, void *client_ip, size_t buf_size, size_t * ip_len)
{
	int ret;
	int type;
	...
	if (getsockopt(bind_ctx->fd, SOL_SOCKET, SO_TYPE,
		       (void *)&type, &type_len) != 0 || (type != SOCK_STREAM && type != SOCK_DGRAM)) {
