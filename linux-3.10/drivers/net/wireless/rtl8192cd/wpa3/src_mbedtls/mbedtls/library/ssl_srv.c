		...
		if (ssl->renego_status == MBEDTLS_SSL_RENEGOTIATION_IN_PROGRESS) {
			...
			if (cli_msg_seq != ssl->handshake->in_msg_seq) {
				MBEDTLS_SSL_DEBUG_MSG(1, ("bad client hello message_seq: "
							  "%d (expected %d)", cli_msg_seq, ssl->handshake->in_msg_seq));
				return (MBEDTLS_ERR_SSL_BAD_HS_CLIENT_HELLO);
			}
...
static void ssl_write_ecjpake_kkpp_ext(mbedtls_ssl_context * ssl, unsigned char *buf, size_t * olen)
{
	...
	*p++ = (unsigned char)((MBEDTLS_TLS_EXT_ECJPAKE_KKPP) & 0xFF);
	...
	ret = mbedtls_ecjpake_write_round_one(&ssl->handshake->ecjpake_ctx,
					      p + 2, end - p - 2, &kkpp_len, ssl->conf->f_rng, ssl->conf->p_rng);
	if (ret != 0) {
...
static int ssl_write_server_hello(mbedtls_ssl_context * ssl)
{
	...
	*p++ = (unsigned char)(t);
	...
	if (ssl->handshake->resume == 0 &&
	    ...
	    ssl->conf->f_get_cache != NULL && ssl->conf->f_get_cache(ssl->conf->p_cache, ssl->session_negotiate) == 0) {
...
static int ssl_get_ecdh_params_from_cert(mbedtls_ssl_context * ssl)
{
	...
	if ((ret = mbedtls_ecdh_get_params(&ssl->handshake->ecdh_ctx,
					   mbedtls_pk_ec(*mbedtls_ssl_own_key(ssl)), MBEDTLS_ECDH_OURS)) != 0) {
...
static int ssl_parse_encrypted_pms(mbedtls_ssl_context * ssl,
				   const unsigned char *p, const unsigned char *end, size_t pms_offset)
{
	int ret;
	...
	mbedtls_ssl_write_version(ssl->handshake->max_major_ver,
				  ssl->handshake->max_minor_ver, ssl->conf->transport, ver);
	...
	ret = mbedtls_pk_decrypt(mbedtls_ssl_own_key(ssl), p, len,
				 peer_pms, &peer_pmslen, sizeof(peer_pms), ssl->conf->f_rng, ssl->conf->p_rng);
...
static int ssl_parse_client_key_exchange(mbedtls_ssl_context * ssl)
{
	...
	if (ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_DHE_RSA) {
		...
		if ((ret = mbedtls_dhm_calc_secret(&ssl->handshake->dhm_ctx,
						   ...
						   &ssl->handshake->pmslen, ssl->conf->f_rng, ssl->conf->p_rng)) != 0) {
	...
	if (ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_ECDHE_RSA ||
		...
		if ((ret = mbedtls_ecdh_calc_secret(&ssl->handshake->ecdh_ctx,
						    ...
						    ssl->handshake->premaster,
						    MBEDTLS_MPI_MAX_SIZE, ssl->conf->f_rng, ssl->conf->p_rng)) != 0) {
...
static int ssl_parse_certificate_verify(mbedtls_ssl_context * ssl)
{
	...
	if ((ret = mbedtls_pk_verify(&ssl->session_negotiate->peer_cert->pk,
				     md_alg, hash_start, hashlen, ssl->in_msg + i, sig_len)) != 0) {
...
static int ssl_write_new_session_ticket(mbedtls_ssl_context * ssl)
{
	...
	if ((ret = ssl->conf->f_ticket_write(ssl->conf->p_ticket,
					     ...
					     ssl->out_msg + 10,
					     ssl->out_msg + MBEDTLS_SSL_MAX_CONTENT_LEN, &tlen, &lifetime)) != 0) {
