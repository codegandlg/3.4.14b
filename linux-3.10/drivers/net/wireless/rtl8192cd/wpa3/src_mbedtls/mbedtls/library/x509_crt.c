...
const mbedtls_x509_crt_profile mbedtls_x509_crt_profile_next = {
	...
	MBEDTLS_X509_ID_FLAG(MBEDTLS_ECP_DP_SECP256R1) |
	    ...
	    MBEDTLS_X509_ID_FLAG(MBEDTLS_ECP_DP_BP512R1) | MBEDTLS_X509_ID_FLAG(MBEDTLS_ECP_DP_SECP256K1),
#else
	0,
#endif
	2048,
...
static int x509_profile_check_key(const mbedtls_x509_crt_profile * profile,
				  mbedtls_pk_type_t pk_alg, const mbedtls_pk_context * pk)
{
...
static int x509_get_subject_alt_name(unsigned char **p,
				     const unsigned char *end, mbedtls_x509_sequence * subject_alt_name)
{
	int ret;
	size_t len, tag_len;
...
static int x509_crt_parse_der_core(mbedtls_x509_crt * crt, const unsigned char *buf, size_t buflen)
{
	...
	if ((ret = mbedtls_x509_get_sig_alg(&crt->sig_oid, &sig_params1,
					    &crt->sig_md, &crt->sig_pk, &crt->sig_opts)) != 0) {
		mbedtls_x509_crt_free(crt);
		return (ret);
	}
...
int mbedtls_x509_crt_parse_path(mbedtls_x509_crt * chain, const char *path)
{
	...
	do {
		...
		w_ret = WideCharToMultiByte(CP_ACP, 0, file_data.cFileName,
					    lstrlenW(file_data.cFileName), p, (int)len - 1, NULL, NULL);
		if (w_ret == 0)
			return (MBEDTLS_ERR_X509_FILE_IO_ERROR);
			      ...
			      mbedtls_x509_crl * crl_list, const mbedtls_x509_crt_profile * profile)
{
	...
	while (crl_list != NULL) {
		if (crl_list->version == 0 ||
		    ...
		    memcmp(crl_list->issuer_raw.p, ca->subject_raw.p, crl_list->issuer_raw.len) != 0) {
...
static int x509_crt_check_parent(const mbedtls_x509_crt * child, const mbedtls_x509_crt * parent, int top, int bottom)
{
	...
	if (top && bottom &&
	    child->raw.len == parent->raw.len && memcmp(child->raw.p, parent->raw.p, child->raw.len) == 0) {
		need_ca_bit = 0;
	}
...
static int x509_crt_verify_top(mbedtls_x509_crt * child, mbedtls_x509_crt * trust_ca,
	...
	*flags |= MBEDTLS_X509_BADCERT_NOT_TRUSTED;
	...
	for ( /* trust_ca */ ; trust_ca != NULL; trust_ca = trust_ca->next) {
		...
		if (child->subject_raw.len == trust_ca->subject_raw.len &&
		    memcmp(child->subject_raw.p, trust_ca->subject_raw.p, child->issuer_raw.len) == 0) {
			check_path_cnt--;
		}
...
static int x509_crt_verify_child(mbedtls_x509_crt * child, mbedtls_x509_crt * parent,
	...
	if (grandparent != NULL) {
		...
		if (grandparent != NULL) {
			ret = x509_crt_verify_child(parent, grandparent, trust_ca, ca_crl,
						    profile, path_cnt + 1, self_cnt, &parent_flags, f_vrfy, p_vrfy);
			if (ret != 0)
				return (ret);
		} else {
	...
	if (cn != NULL) {
		...
		if (crt->ext_types & MBEDTLS_X509_EXT_SUBJECT_ALT_NAME) {
			...
			while (cur != NULL) {
				...
				if (cur->buf.len > 2 &&
				    memcmp(cur->buf.p, "*.", 2) == 0 && x509_check_wildcard(cn, &cur->buf) == 0) {
					break;
				}
