		...
		if (ssl->handshake->verify_cookie == NULL) {
			...
			*p++ = 0;
			...
			MBEDTLS_SSL_DEBUG_BUF(3, "client hello, cookie",
					      ssl->handshake->verify_cookie, ssl->handshake->verify_cookie_len);
...
static int ssl_parse_signature_algorithm(mbedtls_ssl_context * ssl,
					 unsigned char **p,
					 unsigned char *end, mbedtls_md_type_t * md_alg, mbedtls_pk_type_t * pk_alg)
{
	((void)ssl);
...
static int ssl_write_client_key_exchange(mbedtls_ssl_context * ssl)
{
	...
	if (ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_DHE_RSA) {
		...
		ret = mbedtls_dhm_make_public(&ssl->handshake->dhm_ctx,
					      ...
					      &ssl->out_msg[i], n, ssl->conf->f_rng, ssl->conf->p_rng);
		if (ret != 0) {
	...
	if (ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_ECDHE_RSA ||
		...
		ret = mbedtls_ecdh_make_public(&ssl->handshake->ecdh_ctx,
					       &n, &ssl->out_msg[i], 1000, ssl->conf->f_rng, ssl->conf->p_rng);
		if (ret != 0) {
...
static int ssl_write_certificate_verify(mbedtls_ssl_context * ssl)
{
	...
	if ((ret = mbedtls_pk_sign(mbedtls_ssl_own_key(ssl), md_alg, hash_start, hashlen,
				   ssl->out_msg + 6 + offset, &n, ssl->conf->f_rng, ssl->conf->p_rng)) != 0) {
