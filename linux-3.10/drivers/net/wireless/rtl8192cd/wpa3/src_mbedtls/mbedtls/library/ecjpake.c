...
static int ecjpake_zkp_read(const mbedtls_md_info_t * md_info,
			    ...
			    const char *id, const unsigned char **p, const unsigned char *end)
{
	int ret;
...
static int ecjpake_kkpp_read(const mbedtls_md_info_t * md_info,
			     ...
			     mbedtls_ecp_point * Xb, const char *id, const unsigned char *buf, size_t len)
{
	int ret;
...
static int ecjpake_kkpp_write(const mbedtls_md_info_t * md_info,
			      ...
			      const char *id,
			      unsigned char *buf,
			      size_t len, size_t * olen, int (*f_rng) (void *, unsigned char *, size_t), void *p_rng)
{
	int ret;
	unsigned char *p = buf;
	const unsigned char *end = buf + len;
...
int mbedtls_ecjpake_read_round_one(mbedtls_ecjpake_context * ctx, const unsigned char *buf, size_t len)
{
	return (ecjpake_kkpp_read(ctx->md_info, &ctx->grp, ctx->point_format,
				  &ctx->grp.G, &ctx->Xp1, &ctx->Xp2, ID_PEER, buf, len));
}
				    ...
				    int (*f_rng) (void *, unsigned char *, size_t), void *p_rng)
{
	return (ecjpake_kkpp_write(ctx->md_info, &ctx->grp, ctx->point_format,
				   &ctx->grp.G,
				   &ctx->xm1, &ctx->Xm1, &ctx->xm2, &ctx->Xm2, ID_MINE, buf, len, olen, f_rng, p_rng));
}
				    ...
				    int (*f_rng) (void *, unsigned char *, size_t), void *p_rng)
{
	...
	MBEDTLS_MPI_CHK(ecjpake_zkp_write(ctx->md_info, &ctx->grp,
					  ctx->point_format, &G, &xm, &Xm, ID_MINE, &p, end, f_rng, p_rng));
...
static int ecjpake_test_load(mbedtls_ecjpake_context * ctx,
			     const unsigned char *xm1, size_t len1, const unsigned char *xm2, size_t len2)
{
	int ret;
