...
int mbedtls_gcm_starts(mbedtls_gcm_context * ctx,
		       int mode, const unsigned char *iv, size_t iv_len, const unsigned char *add, size_t add_len)
{
	int ret;
			     ...
			     size_t tag_len, const unsigned char *input, unsigned char *output)
{
	...
	if ((ret = mbedtls_gcm_crypt_and_tag(ctx, MBEDTLS_GCM_DECRYPT, length,
					     iv, iv_len, add, add_len, input, output, tag_len, check_tag)) != 0) {
		return (ret);
	}
...
int mbedtls_gcm_self_test(int verbose)
{
	...
	for (j = 0; j < 3; j++) {
		...
		for (i = 0; i < MAX_TESTS; i++) {
			...
			if (ret != 0 ||
			    memcmp(buf, pt[pt_index[i]], pt_len[i]) != 0 || memcmp(tag_buf, tag[j * 6 + i], 16) != 0) {
				if (verbose != 0)
					mbedtls_printf("failed\n");
			...
			ret = mbedtls_gcm_starts(&ctx, MBEDTLS_GCM_ENCRYPT,
						 iv[iv_index[i]], iv_len[i], additional[add_index[i]], add_len[i]);
			if (ret != 0) {
				if (verbose != 0)
					mbedtls_printf("failed\n");
