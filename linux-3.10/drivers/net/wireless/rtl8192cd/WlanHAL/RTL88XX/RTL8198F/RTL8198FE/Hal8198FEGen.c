	...
	)
{
	...
	phydm_txbf_rfmode(ODMPTR, pBeamformingInfo->beamformee_su_cnt, 0);
}
	...
	)
{
	...
	if ((pBeamformingInfo->BeamformCap & BEAMFORMER_CAP) && (BFeeIdx < BEAMFORMEE_ENTRY_NUM)) {
		...
		SetBeamformRfMode8198F(priv, pBeamformingInfo, BFeeIdx);
		...
		if (BFeeIdx == 0) {
			/*Get BIT24 & BIT25*/
			u1Byte	tmp = RTL_R8( REG_ASSOCIATED_BFMEE_SEL_1+3) & 0x3;
			RTL_W8( REG_ASSOCIATED_BFMEE_SEL_1+3, tmp | 0x60);
			RTL_W16( REG_ASSOCIATED_BFMEE_SEL_1, STAid | BIT9);
		} else {
			/*Set BIT25*/
			RTL_W16( REG_ASSOCIATED_BFMEE_SEL_1+2, STAid | 0xE200);
		}
	...
	*/
	if (BeamformeeEntry.BeamformEntryCap == BEAMFORMING_CAP_NONE) {
		if(Idx == 0) {
			...
			RTL_W16( REG_ASSOCIATED_BFMEE_SEL_1, 0);
		} else {
			RTL_W16(REG_TXBF_CTRL+2, RTL_R16( REG_TXBF_CTRL+2) & 0xF000);
			RTL_W16(REG_ASSOCIATED_BFMEE_SEL_1+2,	RTL_R16( REG_ASSOCIATED_BFMEE_SEL_1+2) & 0x60);
		}	
	}
	...
	)
{
	...
	PHYDM_DBG(ODMPTR, DBG_TXBF, "%s Idx %d BeamCtrlReg %x BeamCtrlVal %x, bw=%d\n", __FUNCTION__, Idx, BeamCtrlReg, BeamCtrlVal, BeamformEntry.BW);

}


VOID Beamforming_NDPARate_8198F(
	struct rtl8192cd_priv *priv,
	BOOLEAN		Mode,
	u1Byte		BW,
	u1Byte		Rate
