...
#if (HALMAC_88XX_V1_SUPPORT && HALMAC_USB_SUPPORT)
...
init_usb_cfg_88xx_v1(struct halmac_adapter *adapter)
{
	u8 value8 = 0;
...
/**
 * deinit_usb_cfg_88xx_v1() - deinit USB
 * @adapter : the adapter of halmac
 * Author : KaiYuan Chang/Ivan Lin
 * Return : enum halmac_ret_status
 * More details of status code can be found in prototype document
 */
enum halmac_ret_status
deinit_usb_cfg_88xx_v1(struct halmac_adapter *adapter)
{
	return HALMAC_RET_SUCCESS;
}

/**
 * cfg_usb_rx_agg_88xx_v1() - config rx aggregation
 * @adapter : the adapter of halmac
 * @halmac_rx_agg_mode
 * Author : KaiYuan Chang/Ivan Lin
 * Return : enum halmac_ret_status
 ...
 */
enum halmac_ret_status
cfg_usb_rx_agg_88xx_v1(struct halmac_adapter *adapter,
...
/**
 * reg_r8_usb_88xx_v1() - read 1byte register
 * @adapter : the adapter of halmac
 ...
 */
u8
reg_r8_usb_88xx_v1(struct halmac_adapter *adapter, u32 offset)
{
	return PLTFM_REG_R8(offset);
}

/**
 * reg_w8_usb_88xx_v1() - write 1byte register
 * @adapter : the adapter of halmac
 * @offset : register offset
 ...
 */
enum halmac_ret_status
reg_w8_usb_88xx_v1(struct halmac_adapter *adapter, u32 offset, u8 value)
{
...
/**
 * reg_r16_usb_88xx_v1() - read 2byte register
 * @adapter : the adapter of halmac
 * @offset : register offset
 * Author : KaiYuan Chang/Ivan Lin
 * Return : enum halmac_ret_status
 ...
 */
u16
reg_r16_usb_88xx_v1(struct halmac_adapter *adapter, u32 offset)
{
	return PLTFM_REG_R16(offset);
}

/**
 * reg_w16_usb_88xx_v1() - write 2byte register
 * @adapter : the adapter of halmac
 * @offset : register offset
 * @value : register value
 * Author : KaiYuan Chang/Ivan Lin
 ...
 */
enum halmac_ret_status
reg_w16_usb_88xx_v1(struct halmac_adapter *adapter, u32 offset, u16 value)
{
...
/**
 * reg_r32_usb_88xx_v1() - read 4byte register
 * @adapter : the adapter of halmac
 * @offset : register offset
 * Author : KaiYuan Chang/Ivan Lin
 * Return : enum halmac_ret_status
 ...
 */
u32
reg_r32_usb_88xx_v1(struct halmac_adapter *adapter, u32 offset)
{
	return PLTFM_REG_R32(offset);
}

/**
 * reg_w32_usb_88xx_v1() - write 4byte register
 * @adapter : the adapter of halmac
 * @offset : register offset
 * @value : register value
 * Author : KaiYuan Chang/Ivan Lin
 ...
 */
enum halmac_ret_status
reg_w32_usb_88xx_v1(struct halmac_adapter *adapter, u32 offset, u32 value)
{
...
/**
 * set_usb_bulkout_num_88xx_v1() - inform bulk-out num
 * @adapter : the adapter of halmac
 ...
 */
enum halmac_ret_status
set_usb_bulkout_num_88xx_v1(struct halmac_adapter *adapter, u8 num)
{
...
/**
 * get_usb_bulkout_id_88xx_v1() - get bulk out id for the TX packet
 * @adapter : the adapter of halmac
 ...
 */
enum halmac_ret_status
get_usb_bulkout_id_88xx_v1(struct halmac_adapter *adapter, u8 *buf, u32 size,
			   u8 *id)
{
	enum halmac_qsel queue_sel;
	enum halmac_dma_ch dma_channel;
	u8 ch_id;
	...
	case HALMAC_QSEL_VO_V2:
		dma_channel = adapter->ch_map[HALMAC_PQ_MAP_VO];
		break;
	case HALMAC_QSEL_VI:
	case HALMAC_QSEL_VI_V2:
		dma_channel = adapter->ch_map[HALMAC_PQ_MAP_VI];
		break;
	case HALMAC_QSEL_BE:
	case HALMAC_QSEL_BE_V2:
		dma_channel = adapter->ch_map[HALMAC_PQ_MAP_BE];
		break;
	case HALMAC_QSEL_BK:
	case HALMAC_QSEL_BK_V2:
		dma_channel = adapter->ch_map[HALMAC_PQ_MAP_BK];
		break;
	case HALMAC_QSEL_MGNT:
		dma_channel = adapter->ch_map[HALMAC_PQ_MAP_MG];
		break;
	case HALMAC_QSEL_HIGH:
		dma_channel = HALMAC_DMA_CH_HIGH;
		break;
	case HALMAC_QSEL_CMD:
		dma_channel = HALMAC_DMA_CH_H2C;
		break;
	case HALMAC_QSEL_FWCMD:
		dma_channel = HALMAC_DMA_CH_FWCMD;
		break;
	case HALMAC_QSEL_BCN:
		dma_channel = HALMAC_DMA_CH_BCN;
		break;
	default:
	...
	ch_id = (u8)GET_TX_DESC_CHANNEL_DMA(buf);

	if (dma_channel != ch_id)
		PLTFM_MSG_WARN("[WARN]Channel ID does not match!!\n");

	switch (ch_id) {
	case HALMAC_TXDESC_DMA_CH14:
	case HALMAC_TXDESC_DMA_CH15:
	case HALMAC_TXDESC_DMA_CH19:
	case HALMAC_TXDESC_DMA_CH20:
		*id = 0;
		break;
	case HALMAC_TXDESC_DMA_CH16:
		*id = 1;
		break;
	case HALMAC_TXDESC_DMA_CH0:
		*id = 2;
		break;
	case HALMAC_TXDESC_DMA_CH4:
		*id = 3;
		break;
	case HALMAC_TXDESC_DMA_CH8:
		*id = 4;
		break;
	case HALMAC_TXDESC_DMA_CH12:
		*id = 5;
		break;
	case HALMAC_TXDESC_DMA_CH13:
		*id = 6;
		break;
	default:
...
/**
 * cfg_txagg_usb_align_88xx_v1() -config sdio bus tx agg alignment
 * @adapter : the adapter of halmac
 ...
 */
enum halmac_ret_status
cfg_txagg_usb_align_88xx_v1(struct halmac_adapter *adapter, u8 enable,
			    u16 align_size)
{
...
/**
 * tx_allowed_usb_88xx_v1() - check tx status
 * @adapter : the adapter of halmac
 ...
 */
enum halmac_ret_status
tx_allowed_usb_88xx_v1(struct halmac_adapter *adapter, u8 *buf, u32 size)
{
	return HALMAC_RET_NOT_SUPPORT;
}

/**
 * usb_indirect_reg_r32_88xx_v1() - read MAC reg by SDIO reg
 * @adapter : the adapter of halmac
 * @offset : register offset
 * Author : Soar
 * Return : enum halmac_ret_status
 ...
 */
u32
usb_indirect_reg_r32_88xx_v1(struct halmac_adapter *adapter, u32 offset)
{
	return 0xFFFFFFFF;
}

/**
 * usb_reg_rn_88xx_v1() - read n byte register
 * @adapter : the adapter of halmac
 * @offset : register offset
 ...
 */
enum halmac_ret_status
usb_reg_rn_88xx_v1(struct halmac_adapter *adapter, u32 offset, u32 size,
		   u8 *value)
{
	return HALMAC_RET_NOT_SUPPORT;
}
...
/**
 * get_usb_tx_addr_88xx_v1() - get CMD53 addr for the TX packet
 * @adapter : the adapter of halmac
 * @buf : tx packet, include txdesc
 * @size : tx packet size
 ...
 */
enum halmac_ret_status
get_usb_tx_addr_88xx_v1(struct halmac_adapter *adapter, u8 *buf, u32 size,
			u32 *cmd53_addr)
{
	return HALMAC_RET_NOT_SUPPORT;
}
...
enum halmac_ret_status
set_usb_mode_88xx_v1(struct halmac_adapter *adapter, enum halmac_usb_mode mode)
{
	u32 usb_tmp;
	struct halmac_api *api = (struct halmac_api *)adapter->halmac_api;
...
enum halmac_ret_status
usbphy_write_88xx_v1(struct halmac_adapter *adapter, u8 addr, u16 data,
		     u8 speed)
{
	struct halmac_api *api = (struct halmac_api *)adapter->halmac_api;
...
u16
usbphy_read_88xx_v1(struct halmac_adapter *adapter, u8 addr, u8 speed)
{
	struct halmac_api *api = (struct halmac_api *)adapter->halmac_api;
	u16 value = 0;
...
enum halmac_ret_status
en_ref_autok_usb_88xx_v1(struct halmac_adapter *adapter, u8 en)
{
	return HALMAC_RET_NOT_SUPPORT;
}
