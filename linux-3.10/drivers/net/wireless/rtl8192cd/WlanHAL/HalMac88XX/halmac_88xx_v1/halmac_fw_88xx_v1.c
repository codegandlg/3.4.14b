...
update_fw_info_88xx_v1(struct halmac_adapter *adapter, u8 *fw_bin);

static void
restore_mac_reg_88xx_v1(struct halmac_adapter *adapter,
			struct halmac_backup_info *info, u32 num);

static enum halmac_ret_status
send_fwpkt_88xx_v1(struct halmac_adapter *adapter, u16 pg_addr, u8 *fw_bin,
		   u32 txdesc_size, u32 size);

static enum halmac_ret_status
iddma_dlfw_88xx_v1(struct halmac_adapter *adapter, u32 src, u32 dest, u32 len,
		   u8 first);

static enum halmac_ret_status
iddma_en_88xx_v1(struct halmac_adapter *adapter, u32 src, u32 dest, u32 ctrl);

static enum halmac_ret_status
check_fw_chksum_88xx_v1(struct halmac_adapter *adapter, u32 mem_addr);

static void
fw_fatal_status_debug_88xx_v1(struct halmac_adapter *adapter);

static enum halmac_ret_status
start_dlfw_88xx_v1(struct halmac_adapter *adapter, u8 *fw_bin, u32 size,
		   u32 dl_addr, u8 emem_only);

static enum halmac_ret_status
chk_fw_size_88xx_v1(struct halmac_adapter *adapter, u8 *fw_bin, u32 size);

static void
chk_h2c_ver_88xx_v1(struct halmac_adapter *adapter, u8 *fw_bin);

static void
wlan_cpu_en_88xx_v1(struct halmac_adapter *adapter, u8 enable);

static void
pltfm_reset_88xx_v1(struct halmac_adapter *adapter);

static enum halmac_ret_status
proc_send_phydm_info_88xx_v1(struct halmac_adapter *adapter,
			     struct halmac_general_info *info);

static enum halmac_ret_status
proc_send_general_info_88xx_v1(struct halmac_adapter *adapter,
			       struct halmac_general_info *info);

/**
 * download_firmware_88xx_v1() - download Firmware
...
download_firmware_88xx_v1(struct halmac_adapter *adapter, u8 *fw_bin, u32 size)
{
	u8 value8;
	u32 bckp_idx = 0;
	...
	status = chk_fw_size_88xx_v1(adapter, fw_bin, size);
	if (status != HALMAC_RET_SUCCESS)
		return status;

	chk_h2c_ver_88xx_v1(adapter, fw_bin);

	adapter->halmac_state.dlfw_state = HALMAC_DLFW_NONE;

	wlan_cpu_en_88xx_v1(adapter, 0);
	...
	adapter->ch_map[HALMAC_PQ_MAP_HI] = HALMAC_DMA_CH_0;
	bckp[bckp_idx].length = 1;
	...
	HALMAC_REG_W16_SET(REG_CR, BIT_SHCUT_EN);

	/* Config hi priority queue and public priority queue page number */
	bckp[bckp_idx].length = 2;
	bckp[bckp_idx].mac_register = REG_DMA_RQPN_INFO_PUB;
	bckp[bckp_idx].value = HALMAC_REG_R16(REG_DMA_RQPN_INFO_PUB);
	bckp_idx++;
	bckp[bckp_idx].length = 4;
	bckp[bckp_idx].mac_register = REG_RQPN_CTRL_2_V1;
	bckp[bckp_idx].value = HALMAC_REG_R32(REG_RQPN_CTRL_2_V1) | BIT(31);
	bckp_idx++;
	HALMAC_REG_W16(REG_DMA_RQPN_INFO_PUB, 0x200);
	HALMAC_REG_W32(REG_RQPN_CTRL_2_V1, bckp[bckp_idx - 1].value);

	if (adapter->intf == HALMAC_INTERFACE_SDIO)
		HALMAC_REG_R32(REG_SDIO_FREE_TXPG);

	pltfm_reset_88xx_v1(adapter);

	status = start_dlfw_88xx_v1(adapter, fw_bin, size, 0, 0);

	restore_mac_reg_88xx_v1(adapter, bckp, DLFW_RESTORE_REG_NUM);

	if (status != HALMAC_RET_SUCCESS)
		goto DLFW_FAIL;

	status = dlfw_end_flow_88xx_v1(adapter);
	if (status != HALMAC_RET_SUCCESS)
		goto DLFW_FAIL;
		   ...
		   u32 dl_addr, u8 emem_only)
{
	...
	addr &= ~BIT(31);
	status = dlfw_to_mem_88xx_v1(adapter, cur_fw, 0, addr, dmem_size);
	if (status != HALMAC_RET_SUCCESS)
		return status;
	...
	addr &= ~BIT(31);
	status = dlfw_to_mem_88xx_v1(adapter, cur_fw, 0, addr, imem_size);
	if (status != HALMAC_RET_SUCCESS)
		return status;
...
DLFW_EMEM:
	if (emem_size) {
		...
		addr &= ~BIT(31);
		status = dlfw_to_mem_88xx_v1(adapter, cur_fw, dl_addr << 7,
					     addr, emem_size);
		if (status != HALMAC_RET_SUCCESS)
			return status;
	...
	update_fw_info_88xx_v1(adapter, fw_bin);
...
/**
 * free_download_firmware_88xx_v1() - download specific memory firmware
 * @adapter
 ...
 */
enum halmac_ret_status
free_download_firmware_88xx_v1(struct halmac_adapter *adapter,
			       enum halmac_dlfw_mem mem_sel, u8 *fw_bin,
			       u32 size)
{
	return HALMAC_RET_SUCCESS;
}
...
/**
 * reset_wifi_fw_88xx_v1() - reset wifi fw
 * @adapter : the adapter of halmac
 ...
 */
enum halmac_ret_status
reset_wifi_fw_88xx_v1(struct halmac_adapter *adapter)
{
	PLTFM_MSG_TRACE("[TRACE]%s ===>\n", __func__);
...
/**
 * get_fw_version_88xx_v1() - get FW version
 * @adapter : the adapter of halmac
 * @ver : fw version info
 * Author : Ivan Lin
 * Return : enum halmac_ret_status
 ...
 */
enum halmac_ret_status
get_fw_version_88xx_v1(struct halmac_adapter *adapter,
...
enum halmac_ret_status
dlfw_to_mem_88xx_v1(struct halmac_adapter *adapter, u8 *fw_bin, u32 src,
		    u32 dest, u32 size)
{
	u8 first_part;
	u32 mem_offset;
	...
	u32 pkt_size;
	u16 offset = 0;
	u16 txdesc_size = TX_DESC_BODY_SIZE_88XX_V1;
	u8 ie_body[TX_DESC_BODY_SIZE_88XX_V1] = {0};
	enum halmac_ret_status status;
	struct halmac_api *api = (struct halmac_api *)adapter->halmac_api;
	...
	while (residue_size != 0) {
		...
		status = send_fwpkt_88xx_v1(adapter, (u16)(src >> 7),
					    fw_bin + mem_offset,
					    txdesc_size, pkt_size);
		if (status != HALMAC_RET_SUCCESS) {
			PLTFM_MSG_ERR("[ERR]send fw pkt!!\n");
			return status;
		}

		if (!GET_TX_DESC_TXPKTSIZE(ie_body)) {
			dump_fifo_88xx_v1(adapter, HAL_FIFO_SEL_TX, src,
					  TX_DESC_BODY_SIZE_88XX_V1, ie_body);
			offset = (u16)GET_TX_DESC_OFFSET(ie_body);
			txdesc_size += (u16)GET_TX_DESC_PKT_OFFSET_V1(ie_body)
					<< 3;
			if (offset < TX_DESC_BUF_SIZE_88XX_V1) {
				PLTFM_MSG_ERR("[ERR]txdesc offset!!\n");
				return HALMAC_RET_TXDESC_SET_FAIL;
			}
		}

		status = iddma_dlfw_88xx_v1(adapter,
					    OCPBASE_TXBUF_88XX_V1 + src +
					    offset,
	...
	status = check_fw_chksum_88xx_v1(adapter, dest);
	if (status != HALMAC_RET_SUCCESS) {
			...
			struct halmac_backup_info *info, u32 num)
{
	...
	u32 value;
	struct halmac_api *api = (struct halmac_api *)adapter->halmac_api;
	struct halmac_backup_info *cur_info = info;

	for (i = 0; i < num; i++) {
		reg = cur_info->mac_register;
...
enum halmac_ret_status
dlfw_end_flow_88xx_v1(struct halmac_adapter *adapter)
{
	u16 fw_ctrl;
	u32 cnt;
	struct halmac_api *api = (struct halmac_api *)adapter->halmac_api;
		   ...
		   u32 txdesc_size, u32 size)
{
	...
	status = dl_rsvd_page_88xx_v1(adapter, pg_addr, fw_bin, size);
	if (status != HALMAC_RET_SUCCESS)
		   ...
		   u8 first)
{
	...
	if (iddma_en_88xx_v1(adapter, src, dest, ch0_ctrl) !=
	    HALMAC_RET_SUCCESS) {
...
check_fw_chksum_88xx_v1(struct halmac_adapter *adapter, u32 mem_addr)
{
	...
	if (HALMAC_REG_R32(REG_DDMA_CH0CTRL) & BIT_DDMACH0_CHKSUM_STS) {
		if (mem_addr < OCPBASE_DMEM_88XX_V1) {
...
/**
 * check_fw_status_88xx_v1() -check fw status
 * @adapter : the adapter of halmac
 * @fw_status : fw status
 * Author : KaiYuan Chang/Ivan Lin
 * Return : enum halmac_ret_status
 ...
 */
enum halmac_ret_status
check_fw_status_88xx_v1(struct halmac_adapter *adapter, u8 *fw_status)
{
	u32 cnt;
	u32 fw_dbg6;
	u32 fw_pc;
	...
	*fw_status = 1;
	...
	if (fw_dbg6 != 0) {
		...
		if ((fw_dbg6 & FW_STATUS_CHK_FATAL) != 0) {
			...
			fw_fatal_status_debug_88xx_v1(adapter);
			*fw_status = 0;
			return status;
		}
	}
...
enum halmac_ret_status
dump_fw_dmem_88xx_v1(struct halmac_adapter *adapter, u8 *dmem, u32 *size)
{
	return HALMAC_RET_SUCCESS;
}

/**
 * cfg_max_dl_size_88xx_v1() - config max download FW size
 * @adapter : the adapter of halmac
 ...
 */
enum halmac_ret_status
cfg_max_dl_size_88xx_v1(struct halmac_adapter *adapter, u32 size)
{
	PLTFM_MSG_TRACE("[TRACE]%s ===>\n", __func__);
...
/**
 * enter_cpu_sleep_mode_88xx_v1() -wlan cpu enter sleep mode
 * @adapter : the adapter of halmac
 * Author : Ivan Lin
 * Return : enum halmac_ret_status
 * More details of status code can be found in prototype document
 */
enum halmac_ret_status
enter_cpu_sleep_mode_88xx_v1(struct halmac_adapter *adapter)
{
	u32 cnt;
	struct halmac_api *api = (struct halmac_api *)adapter->halmac_api;
...
/**
 * get_cpu_mode_88xx_v1() -get wlcpu mode
 * @adapter : the adapter of halmac
 * @mode : cpu mode
 * Author : Ivan Lin
 * Return : enum halmac_ret_status
 ...
 */
enum halmac_ret_status
get_cpu_mode_88xx_v1(struct halmac_adapter *adapter,
...
/**
 * send_general_info_88xx_v1() -send general information to FW
 * @adapter : the adapter of halmac
 ...
 */
enum halmac_ret_status
send_general_info_88xx_v1(struct halmac_adapter *adapter,
			  struct halmac_general_info *info)
{
	u8 h2cq_ele[4] = {0};
	u32 h2cq_addr;
	...
	if (adapter->fw_ver.h2c_version < 14)
		PLTFM_MSG_WARN("[WARN]the info format does not match halmac\n");
	...
	status = proc_send_general_info_88xx_v1(adapter, info);
	if (status != HALMAC_RET_SUCCESS) {
		PLTFM_MSG_ERR("[ERR]send gen info!!\n");
		return status;
	}

	status = proc_send_phydm_info_88xx_v1(adapter, info);
	if (status != HALMAC_RET_SUCCESS) {
	...
	h2cq_addr <<= TX_PAGE_SIZE_SHIFT_88XX_V1;

	cnt = 100;
	do {
		status = dump_fifo_88xx_v1(adapter, HAL_FIFO_SEL_TX,
...
/**
 * drv_fwctrl_88xx_v1() - send drv-defined h2c pkt
 * @adapter : the adapter of halmac
 ...
 */
enum halmac_ret_status
drv_fwctrl_88xx_v1(struct halmac_adapter *adapter, u8 *payload, u32 size,
		   u8 ack)
{
