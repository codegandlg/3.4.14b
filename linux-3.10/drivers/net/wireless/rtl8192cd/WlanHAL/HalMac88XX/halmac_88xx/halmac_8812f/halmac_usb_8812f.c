...
/**
 * mac_pwr_switch_usb_8812f() - switch mac power
		  ...
		  enum halmac_intf_phy_platform pltfm)
{
	...
	status = parse_intf_phy_88xx(adapter, usb2_phy_param_8812f, pltfm,
				     HAL_INTF_PHY_USB2);

	if (status != HALMAC_RET_SUCCESS)
		return status;

	status = parse_intf_phy_88xx(adapter, usb3_phy_param_8812f, pltfm,
				     HAL_INTF_PHY_USB3);
...
/**
 * halmac_pcie_switch_8812f() - pcie gen1/gen2 switch
 * @adapter : the adapter of halmac
...
/**
 * intf_tun_usb_8812f() - usb interface fine tuning
 * @adapter : the adapter of halmac
 * Author : Ivan
 * Return : enum halmac_ret_status
 * More details of status code can be found in prototype document
