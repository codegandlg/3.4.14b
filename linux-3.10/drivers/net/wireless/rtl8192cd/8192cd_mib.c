...
struct mib_val dot11_mib_table[]={
	...
	{ {2, 1, 1, 19, 1, -1}, "dot11BlockWlanRelay", MIB_TYPE_PRIV,PRIV_OFFSET(dot11OperationEntry.block_wlan_relay), PRIV_SIZE(dot11OperationEntry.block_wlan_relay)},
...
int mib_get(struct rtl8192cd_priv *priv, char *oid, unsigned char *data, unsigned int data_len, int *pLen)
{
 	int iVal;
	...
	*pVal_type = VAL_OCTET;
	...
	switch (mib->type) {
		case MIB_TYPE_PRIV:
			...
			if (*pLen > data_len) {
				printk("mib_get Error: mib size (%d) exceeds!\n", *pLen);
				return 0;
			}
