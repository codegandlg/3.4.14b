...
#ifdef CONFIG_RTL_819X
//only forward packet or dns request packet send from DUT
int limit_trigger_packet(struct sk_buff *skb, int offset)
{
	int ret_val=0;
	if (skb->sk)  //packet send from DUT
	{	
		if ( (((u8*)skb->data)[offset-2]==0x00 && ((u8*)skb->data)[offset-1] == 0x21)    /* IPV4 */
		&& (((u8*)skb->data)[9+offset]==0x11) 					/* UDP */
		&& (((u8*)skb->data)[22+offset]==0x00 && ((u8*)skb->data)[23+offset] == 0x35) /* DNS query*/
		)
		{			
			//printk("\n%s:%d send IPV4 dns request!!!\n",__FUNCTION__,__LINE__);
			ret_val=0;	
		}
		else if ( (((u8*)skb->data)[offset-2]==0x00 && ((u8*)skb->data)[offset-1] == 0x57)    /* IPV6 */
		&& (((u8*)skb->data)[6+offset]==0x11) 					/* UDP */
		&& (((u8*)skb->data)[42+offset]==0x00 && ((u8*)skb->data)[43+offset] == 0x35) /* DNS query*/
		)
		{			
...
{	
	//struct iphdr *iphp;
	//iphp = (struct iphdr *)((unsigned char *)(skb->data+offset));
	...
	if(iphp->protocol == IPPROTO_ICMP)
	{
		struct icmphdr *icmph= (void *)iphp + iphp->ihl*4;
	...
	else if(iphp->protocol == IPPROTO_TCP)
	{
		struct tcphdr *tcph;
		tcph = (void *)iphp + iphp->ihl*4;
	...
	else if(iphp->protocol == IPPROTO_IGMP)
	{
