...
static rtl865x_vlan_entry_t *rtl_findEmptyVlanEntry(uint16 vid)
{
	int i;
	rtl865x_vlan_entry_t *entry = NULL;
	...
	for(i=0; i<VLANTBL_SIZE; i++)
	{
		//if vlan entry already exist, use this vlan entry. otherwis use empty vlan entry.
		if(vlanTbl[i].valid && (vlanTbl[i].vid == vid)){
			entry = &vlanTbl[i];
			break;
		}
...
#if defined(CONFIG_RTL_8196E) || defined(CONFIG_RTL_819XD)	
	entry = rtl_findEmptyVlanEntry(vid);
	if(entry == NULL)
		return RTL_EINVALIDVLANID;
#else
	entry=&vlanTbl[vid];
