...
#if defined(CONFIG_RTL_8197F) && defined(CONFIG_RTL_SWITCH_NEW_DESCRIPTOR)
#undef __IRAM_FWD
#define __IRAM_FWD
#endif
		...
		{	
			...
			*/
			...
			if(	m_pkthdr->ph_extPortList&&
				(5!=extPortMaskToPortNum[(m_pkthdr->ph_extPortList)&0x7]))
			{
			...
			if(extPortEnabled)
			{
				...
				else
				{
					...
					*vid = PKTHDR_EXTPORT_MAGIC4;
					srcPortNum=extPortMaskToPortNum[(m_pkthdr->ph_extPortList)&0x7];
...
__IRAM_FWD  inline int32 _swNic_send(void *skb, void * output, uint32 len,rtl_nicTx_info *nicTx)
{
	...
	pPkthdr->ph_txPriority = 0x7 &	rtk_get_vlan_priority(pPkthdr->ph_vlanId);
