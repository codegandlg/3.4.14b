...
static uint32 rtl8198c_ipv6ArpHash(inv6_addr_t ip, uint32 subnetIdx)
{
	...
	memset(arpv6_hash_idx, 0, sizeof(arpv6_hash_idx));
	memset(arpv6_hash, 0, sizeof(arpv6_hash));
...
int32 rtl8198c_getIpv6ArpMapping(inv6_addr_t ip, rtl8198c_ipv6_arpMapping_entry_t * arp_mapping)
{
	...
	if ((route->valid!=1) ||((route->process!=2)&&(route->process!=5))  ||(route->dstNetif==NULL))
		return FAILED;
...
static int32 rtl8198c_isLinklocal(inv6_addr_t ip)
{
	if(ip.v6_addr16[0]==0xfe80)
		return 1;
	else
		return 0;
}
...
#if 1
static int32 rtl8198c_getIpv6ArpFidByIfname(char *ifName, uint16 *fid)
{
	rtl865x_netif_local_t *netif = NULL;
	rtl865x_vlan_entry_t *vlan;
	if(!ifName) return FAILED;
	
		
	netif = _rtl865x_getSWNetifByName(ifName);
	if(!netif) return FAILED;
	vlan = _rtl8651_getVlanTableEntry(netif->vid);
	if(!vlan) return FAILED;
	*fid = vlan->fid;
	return SUCCESS;
...
int32 rtl8198c_addIpv6Arp(inv6_addr_t ip, ether_addr_t *mac,int8 * ifName)
{
	...
	#if 0
	printk("%s.%d.ipAddr(0x%4x%4x%4x%4x%4x%4x%4x%4x),mac(%02x:%02x:%02x:%02x:%02x:%02x) ifname=%s****\n",
			__FUNCTION__,__LINE__,
			...
			mac->octet[2],mac->octet[3],mac->octet[4],mac->octet[5],ifName);
	#endif
	...
	/*Skip linklocal ARP*/	
	if(((ip.v6_addr32[0] >> 16) & 0xFFFF) == 0xFE80)
	{
		return RTL_EINVALIDINPUT;
	}
...
#if 1
					if(rtl8198c_isLinklocal(ip)){
						if(rtl8198c_getIpv6ArpFidByIfname(ifName,&fid)==FAILED)
							return FAILED;
					}else
#endif
	...
	if(ifName)
		memcpy(newArpMapping.name,ifName,sizeof(MAX_IFNAMESIZE));
	...
	for (i=0; i<2; i++) 
	{
		...
		#if 0
		printk("%s:%d\n,fid(%d),mac(%02x:%02x:%02x:%02x:%02x:%02x) fid=%d\n",__FUNCTION__,__LINE__,fid,mac->octet[0],mac->octet[1],
			mac->octet[2],mac->octet[3],mac->octet[4],mac->octet[5],fid);
		#endif
		...
		asicArpEntry.aging    =    31;  /* asic arp entry age time only 5 bits */
...
int32 rtl8198c_delIpv6ArpBySubnetIdx(uint32 subnetIdx)
{
	...
	for (i=0; i<RTL8198C_IPV6_ARPTBL_SIZE; i++)
	{
		if ((ipv6ArpTables.mappings[i].valid==1) && (ipv6ArpTables.mappings[i].subnetIdx==subnetIdx)) {
			...
			if(rtl8198c_isLinklocal(ipv6ArpTables.mappings[i].ip)){
				if(rtl8198c_getIpv6ArpFidByIfname(ipv6ArpTables.mappings[i].name,&fid)==FAILED)
					continue;
			}
			rtl8198C_delAsicArpV6(i);
...
int rtl819x_restore_hw_ipv6_arp_table(void)
{
	...
	for (i = 0; i < RTL8198C_IPV6_ARPTBL_SIZE; i++)
	{
		if ((ipv6ArpTablesBack.mappings[i].ip.v6_addr32[0]&& ipv6ArpTablesBack.mappings[i].ip.v6_addr32[1]
			...
			ret = rtl8198c_addIpv6Arp(ipv6ArpTablesBack.mappings[i].ip, &ipv6ArpTablesBack.mappings[i].mac,ipv6ArpTablesBack.mappings[i].name);
		}
	}
