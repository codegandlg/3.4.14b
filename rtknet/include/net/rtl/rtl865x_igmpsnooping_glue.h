...
#define SMP_LOCK_IGMP_CONCURRENT(__x__)	\
	do { \
		if(lock_owner_igmp_concurrent!=smp_processor_id()) \
			spin_lock_irqsave(&lock_igmp_concurrent, (__x__)); \
		else \
			rtlglue_printf("[%s %s %d] recursion detection in IGMP CONCURRENT\n",__FILE__, __FUNCTION__,__LINE__); \
		lock_owner_igmp_concurrent=smp_processor_id();\
	} while(0)

#define SMP_UNLOCK_IGMP_CONCURRENT(__x__) \
	do { \
		lock_owner_igmp_concurrent=-1; \
		spin_unlock_irqrestore(&lock_igmp_concurrent, (__x__)); \
	} while(0)
