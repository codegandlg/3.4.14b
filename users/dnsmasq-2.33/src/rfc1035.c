...
int rtl_check_dns_reply(HEADER *header, size_t qlen, char *name)
{
	unsigned char *p, *p1;
	int i, j, qtype, qclass, aqtype, aqclass;
	...
	if(ntohs(header->qdcount)==1 && ntohs(header->ancount)==1)
	{
		p = (unsigned char *)(header+1);
		...
		if (!(extract_name(header, qlen, &p1, name, 0)))
				return -1; 
		...
		if ( aqtype != qtype)
		{
			printf("\n%s:%d reply's type does not match the request's type!!!\n",__FUNCTION__,__LINE__);
			return -1;
		}		
	}
