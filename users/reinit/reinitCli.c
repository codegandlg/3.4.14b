...
int reinit_Calledfrom_reinitSer(void)
{
	pid_t	pid,ppid;
	FILE	*file;
	char buff[128] = {0};
	...
	while(pid != 1){	
		snprintf(cmdbuff,64,"cat /proc/%d/status",pid);
		...
		if (NULL != file)
	    {
	        while (fgets(buff, sizeof(buff), file) != NULL)
	        {
	        	if(strstr(buff,"Name:")){
					...
					if(strcmp("reinitSer",name) == 0){
						pclose(file);
						return 1;	
					}
				}
	        	if(strstr(buff,"PPid:")){
					sscanf(buff,"PPid:\t%d",&ppid);
					break;
	        	}
	        }
			pid=ppid;
...
int main(int argc, char **argv)
{
	...
	ret = reinit_Calledfrom_reinitSer();
	if(ret < 0){
		fprintf(stderr,"get pid=%d parenet fail\n",getpid());
		return -1;
	}

	if(ret){
		printf("pid=%d called from reinitSer!!!\n",getpid());
		if(daemon(0,1) == -1){
