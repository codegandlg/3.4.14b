...
static void dump_Ctx(CTX_Tp pCtx)
{
	int i;
	
	/* eth */
	if (pCtx->socket)
		printf(COLOR_CYAN"eth socket[PF_PACKET] succesed\n"COLOR_NONE);
	else
		printf(COLOR_CYAN"eth socket[PF_PACKET] failed\n"COLOR_NONE);

	printf(COLOR_CYAN"eth_intf_name = %s, eth_intf_index = %d\n"COLOR_NONE, pCtx->eth_intf_name, pCtx->eth_intf_name);
	printf(COLOR_CYAN"pCtx->own_addr: %02x:%02x:%02x:%02x:%02x:%02x "COLOR_NONE, MAC2STR(pCtx->own_addr));

	/* wlan */
	if (pCtx->wlan_socket)
		printf(COLOR_CYAN"wlan socket[PF_PACKET] succesed\n"COLOR_NONE);
	else
		printf(COLOR_CYAN"wlan socket[PF_PACKET] failed\n"COLOR_NONE);

	
	for (i=0; i<pCtx->wlan_intf_num; i++)
	{
		printf(COLOR_CYAN"wifi: %s (%d/%d)\n"COLOR_NONE, pCtx->wlan_intf_name[i], i+1, pCtx->wlan_intf_num);
		printf(COLOR_CYAN"  %02x:%02x:%02x:%02x:%02x:%02x\n"COLOR_NONE, MAC2STR(pCtx->wlan_intf_addr[i]));
	}	

	/* pid */
	printf(COLOR_CYAN"pid: %s\n"COLOR_NONE, pCtx->pid_filename);

	/* kh */
	dump_KH(pCtx);
	printf(COLOR_CYAN"config: %s\n"COLOR_NONE, pCtx->config_filename);

	/* other */
	printf(COLOR_CYAN"debug level = %d\n"COLOR_NONE, pCtx->debug_level);
}
