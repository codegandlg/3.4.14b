...
static int get_beacon_measurement_report(char *ifname, unsigned char * macaddr,
    enum MEASUREMENT_RESULT* measure_result, int * bss_num,
                           ...
                           char * bssid_str, char * ssid_str, unsigned char report_detail)
{
    ...
    if(0 == issue_beacon_measurement(ifname, macaddr, &beacon_req))
    {
        ...
        while(ret == 0 && i < BEACON_MEASURE_TIME && (measure_result == MEASUREMENT_PROCESSING || measure_result == MEASUREMENT_RECEIVED ));
    }
...
static int get_link_measurement_report(char *ifname, unsigned char * macaddr, 
    enum MEASUREMENT_RESULT* measure_result, struct dot11k_link_measurement * link_report)
{
    int sock;
    struct iwreq wrq;
    int ret = -1;
...
static void do_link_measurement(char *ifname, char * macaddr_str)
{
    ...
    int i;
    unsigned char macaddr[MACADDRLEN];
    enum MEASUREMENT_RESULT measure_result = MEASUREMENT_UNKNOWN;
...
static int get_neighbor_report(char *ifname, enum MEASUREMENT_RESULT* measure_result, int * bss_num,
