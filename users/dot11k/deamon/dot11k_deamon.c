...
//#define _WNM_BSS_TRANS_CANDIDATE_PREFRENCE_ 3 //tesia
...
struct dot11k_sta_info
{
    ...
    enum MEASUREMENT_RESULT result;
};
...
void TimerFunc()
{
    ...
    for(j = 0; j < BEACON_MEASURE_TIME; j++)
    {
        ...
        for(i = 0; i < MAX_STA_NUM; i++)
        {
            ...
            {
                if(dot11k_sta[i].result == MEASUREMENT_PROCESSING || dot11k_sta[i].result == MEASUREMENT_RECEIVED)
                {
