...
unsigned char MULTIAP_BACKHAUL_STA[6] = {0x00, 0x37, 0x2A, 0x06, 0x01, 0x80};
#endif
...
int wlioctl_set_led(int flag)
{
	...
	if (flag == TURNKEY_LED_WSC_END) { // for turnkey LED_WSC_END
		if(status_code==PROTOCOL_SUCCESS){
...
int wlioctl_get_mib_byte_map(char *interfacename, char *mibname, unsigned char *result, int size)
{
	...
	/* Do the request */
	if (ioctl(skfd, SIOCGIWRTLGETMIB, &wrq) < 0) {
		WSC_DEBUG("ioctl [SIOCGIWRTLGETMIB] for mib %s on %s failed!\n", mibname, interfacename);
		close(skfd);
		return -1;
	}

	if (size > wrq.u.data.length) {
...
int get_mac_address_map(char *interface, unsigned char *addr)
{
	...
	skfd = socket(PF_PACKET, SOCK_RAW, IPPROTO_RAW);
	if (skfd < 0) {
		perror("socket error\n");
		return -1;
	}
...
void client_set_WlanDriver_WscEnable(const CTX_Tp pCtx, const int wps_enabled)
{
...
#endif
	{
		if(pCtx->encrypt_type == WSC_ENCRYPT_WEP) {
			if(wps_enabled)
				sprintf(tmpbuf,"iwpriv %s set_mib encmode=0", pCtx->wlan_interface_name);
			else
				sprintf(tmpbuf,"iwpriv %s set_mib encmode=1", pCtx->wlan_interface_name);
			system(tmpbuf);
		}
	...
	if(pCtx->wlan1_wsc_disabled == 0) {
		if(pCtx->encrypt_type2 == WSC_ENCRYPT_WEP) {
			if(wps_enabled)
				sprintf(tmpbuf,"iwpriv %s set_mib encmode=0", pCtx->wlan_interface_name2);
			else
				sprintf(tmpbuf,"iwpriv %s set_mib encmode=1", pCtx->wlan_interface_name2);
			system(tmpbuf);
		}
...
int derive_key(CTX_Tp pCtx, STA_CTX_Tp pSta)
{
	...
	if (size < PUBLIC_KEY_LEN) {
		add_prefix_padding(pSta->dh_shared_key, size);
		size = PUBLIC_KEY_LEN;
	}
...
#ifdef FOR_DUAL_BAND
	if(pCtx->InterFaceComeIn == COME_FROM_WLAN1)
		memcpy(&pCtx->BackupTagAPBSSID , &pCtx->ss_status2.bssdb[i2].bssid,6);
	else
#endif
		memcpy(&pCtx->BackupTagAPBSSID , &pCtx->ss_status.bssdb[i2].bssid,6);
...
scanClonedAP:
	...
	if(pCtx->InterFaceComeIn == COME_FROM_WLAN1) {
		for (i3=0; i3<pCtx->ss_status2.number && pCtx->ss_status2.number!=0xff; i3++) {
			if( memcmp( &pCtx->BackupTagAPBSSID , &pCtx->ss_status2.bssdb[i3].bssid ,6)==0) {
				pCtx->join_idx = i3;
				found = 1;
...
#endif
	{
		for (i3=0; i3<pCtx->ss_status.number && pCtx->ss_status.number!=0xff; i3++) {
			if( memcmp( &pCtx->BackupTagAPBSSID , &pCtx->ss_status.bssdb[i3].bssid ,6)==0) {
				pCtx->join_idx = i3;
				found = 1;
				UTIL_DEBUG("\n Target	Cloned AP found!!!\n\n");
				break;
...
int write_param_to_flash(CTX_Tp pCtx, int is_local)
{
	...
	if (is_local) {
		...
		if(pCtx->is_ap) {
			...
			if (pCtx->assigned_auth_type & WSC_AUTH_WPA3SAE)
				encrypt = 16;
		} else {
			if (pCtx->assigned_auth_type & WSC_AUTH_WPAPSK)
				encrypt = 2;
	...
	if (is_local) {
		...
		if (pCtx->assigned_auth_type & WSC_AUTH_WPAPSK || pCtx->assigned_auth_type & WSC_AUTH_WPA2PSK  || pCtx->assigned_auth_type & WSC_AUTH_WPA3SAE)
			is_psk = 1;
	}
					   ...
					   unsigned short method, unsigned char *data)
{
	...
	0x0280 <A1>V The APUT cannot use push button to add an External Registrar. If this option is set this test must FAIL.
	0x0480 <A1>V The APUT cannot use push button to add an External Registrar. If this  option is set this test must FAIL.
	*/
	shortVal2 = (pCtx->config_method & ~(CONFIG_METHOD_VIRTUAL_PBC | CONFIG_METHOD_PHYSICAL_PBC)) ;
#endif
...
unsigned char search_blocked_ap_list(CTX_Tp pCtx, int idx, int selected)
{
	...
	for (i=0; i<MAX_BLOCKED_AP_NUM; i++) {
		...
		if (!pCtx->blocked_ap_list[i].used)
			...
			if (!memcmp(pCtx->blocked_ap_list[i].addr, pBss->bssid, ETHER_ADDRLEN)) {
...
void add_into_blocked_ap_list(CTX_Tp pCtx, int idx, int selected)
{
	...
	for (i=0; i<MAX_BLOCKED_AP_NUM; i++) {
		if (pCtx->blocked_ap_list[i].used == 0) {
			...
			memcpy(pCtx->blocked_ap_list[i].addr, pBss->bssid, ETHER_ADDRLEN);
...
void add_prefix_padding(unsigned char *dst, int size)
{
	unsigned char buf[PUBLIC_KEY_LEN];

	memcpy(buf, dst, size);
	memset(dst, 0, PUBLIC_KEY_LEN-size);
	memcpy(dst+(PUBLIC_KEY_LEN-size), buf, size);
}

#ifdef TR069_SUPPORT
#ifndef __ECOS
int write_param_to_flash_for_tr069(char *ifname, void *value, int writeflag)
{
	char tmpbuf[200];
	FILE *fp;
	extRegInfoTp pextReg = NULL;
	unsigned int setValue = 0, lock = 0, pinReach = 0;

	if(!(writeflag & FLAG_ER_DECRALL)) {
		if(value==NULL) {
			UTIL_DEBUG("NULL value \n");
			return -1;
		}
	}
	fp = fopen(PARAM_TEMP_FILE_tr069, "w");
	if(fp==NULL) {
		UTIL_DEBUG("open %s fail !\n", PARAM_TEMP_FILE_tr069);
		return -1;
	} else {
		UTIL_DEBUG("ifname:%s\n", ifname);
		switch(writeflag) {
			case FLAG_ERRCODE: // last config error
				sprintf(tmpbuf, "WSC_LAST_CONFIG_ERR=%d\n", *(int *)value);
				UTIL_DEBUG("errCode :%d\n", *(int *)value);
				fputs(tmpbuf, fp);
				break;
			case (FLAG_EXTREG | FLAG_ER_INCR):
			case (FLAG_EXTREG | FLAG_ER_DECR):
			case (FLAG_EXTREG | FLAG_ER_DECR | FLAG_ER_DECRALL):
				pextReg = (extRegInfoTp)value;
				if(writeflag & FLAG_ER_INCR) {
					sprintf(tmpbuf, "WSC_ER_ADD=%d %s %s\n", pextReg->disable, pextReg->uuid, pextReg->devicename); // add value
					UTIL_DEBUG("[ADD]extReg :%s\n", tmpbuf);
				} else if(writeflag & FLAG_ER_DECR) {
					if(writeflag & FLAG_ER_DECRALL) {
						sprintf(tmpbuf, "WSC_ER_DELALL=1\n");  // del all
						UTIL_DEBUG("[DELALL]extReg :%s\n", tmpbuf);
					} else {
						sprintf(tmpbuf, "WSC_ER_DEL=%d %s %s\n", pextReg->disable, pextReg->uuid, pextReg->devicename);  // del value
						UTIL_DEBUG("[DEL]extReg :%s\n",  tmpbuf);
					}
				}
				fputs(tmpbuf, fp);
				break;
			case FLAG_LOCK_DOWN:
				...
				sprintf(tmpbuf, "WSC_AUTO_LOCK_DOWN=%d\n", lock);
				fputs(tmpbuf, fp);

				sprintf(tmpbuf, "WSC_LOCKDOWN_PIN_REACHED=%d\n", pinReach);
				fputs(tmpbuf, fp);

				UTIL_DEBUG("lockdown :%d, pinReach:%d\n", lock, pinReach);
				break;
			default:
				UTIL_DEBUG("Invalid Writeflag\n");
				break;
		}
		fclose(fp);
		sprintf(tmpbuf, "%s -param_file_tr069 %s %s", WRITE_FLASH_PROG, ifname, PARAM_TEMP_FILE_tr069);
		system(tmpbuf);
		return 0;
	}
}
...
#ifdef __ECOS
int set_lock_down_stat_one_interface(CTX_Tp pCtx, unsigned int lock, unsigned int pinReach, char *ifname)
{
	...
	apmib_save_idx();
#endif
	UTIL_DEBUG("ifname:%s, lock:%d, pin:%d\n", ifname, lock, pinReach);
	SetWlan_idx(ifname);
	apmib_set(MIB_WLAN_WSC_AUTO_LOCK_DOWN, (void *)&lock);
	apmib_set(MIB_WLAN_WSC_LOCKDOWN_PIN_REACHED, (void *)&pinReach);
...
int set_last_config_err_one_interface(CTX_Tp pCtx, unsigned int errFlag, char *ifname)
{
	/* save wlan index - current wlan */
#ifdef CONFIG_WLANIDX_MUTEX
	int s = apmib_save_idx();
#else
	apmib_save_idx();
	...
	SetWlan_idx(ifname);
	apmib_set(MIB_WLAN_WSC_LAST_CONFIG_ERR,(void *)&errFlag);
...
int set_external_registrar_info_one_interface(CTX_Tp pCtx, STA_CTX_Tp pSta, unsigned int flag, char *ifname)
{
	char uuidtmp[UUID_LEN*2+1] = {0};
	unsigned int maxEntryNum = pCtx->ER_num;
	int j = 0, found = 0;
	extRegInfoT uuidEntrytmp, extReg;
	...
	if(flag & FLAG_ER_DECRALL) {
		apmib_set(MIB_WLAN_WSC_ER_DELALL, (void *)&extReg);
	} else if(flag & FLAG_ER_INCR || flag & FLAG_ER_DECR) {
		convert_bin_to_str(pSta->uuid, 16, uuidtmp);
		snprintf(extReg.uuid, UUID_LEN*2+1, "%s" , uuidtmp);
		snprintf(extReg.devicename, MAX_DEVICE_NAME_LEN, "%s" , pSta->devname);
		extReg.disable = pSta->extRegDisable;

		UTIL_DEBUG("extReg disable:%d\n", extReg.disable);
		UTIL_DEBUG("uuid:%s (len:%d)\n", extReg.uuid, strlen(extReg.uuid));
		UTIL_DEBUG("devname:%s (len:%d)\n", extReg.devicename, strlen(extReg.devicename));

		if(flag & FLAG_ER_INCR) {
			//add same rule check
			for(j=1; j<=maxEntryNum; j++) {
				...
				*((char *)&uuidEntrytmp) = (char)j;
				if ( apmib_get(MIB_WLAN_WSC_ER_TBL, (void *)&uuidEntrytmp)) {
					UTIL_DEBUG("tmp uuid:%s\n", uuidEntrytmp.uuid);
					if (!strcmp(uuidEntrytmp.uuid, extReg.uuid)) {
						UTIL_DEBUG("entry already exist!\n");
						goto exit;
					}
				}
			}
			// set to MIB. try to delete it first to avoid duplicate case
			apmib_set(MIB_WLAN_WSC_ER_DEL, (void *)&extReg);
			if ( apmib_set(MIB_WLAN_WSC_ER_ADD, (void *)&extReg) == 0) {
				UTIL_DEBUG("Add table entry error!\n");
				goto exit;
			}
		} else if(flag & FLAG_ER_DECR) {
			...
			for(j=1; j<=maxEntryNum; j++) {
				...
				*((char *)&uuidEntrytmp) = (char)j;
				if ( apmib_get(MIB_WLAN_WSC_ER_TBL, (void *)&uuidEntrytmp)) {
					UTIL_DEBUG("tmp uuid:%s\n", uuidEntrytmp.uuid);
					if (!strcmp(uuidEntrytmp.uuid, extReg.uuid)) {
						UTIL_DEBUG("find the existed entry!\n");
						found = 1;
						break;
					}
				}
			}
			if( j>=maxEntryNum && !found ) {
				UTIL_DEBUG("the to be deleted entry not exist!\n");
				goto exit;
			}
			// delete the specified entry
			if(apmib_set(MIB_WLAN_WSC_ER_DEL, (void *)&extReg) == 0) {
				UTIL_DEBUG("Del table entry error!\n");
				goto exit;
			}
		}
	}
...
int set_lock_down_stat(CTX_Tp pCtx, unsigned int inValue)
{
#ifdef __ECOS
	unsigned int lock = 0, pinReach = 0;
	char ifname_next[] = {0};
	int len = 0;
	switch(inValue) {
		case 0:
		...
		case 2:
			...
			break;
		default:
			UTIL_DEBUG("invalid value\n");
			break;
	}
	UTIL_DEBUG("lock: %d, pinReach:%d\n", lock, pinReach);
	set_lock_down_stat_one_interface(pCtx, lock, pinReach, "wlan0");
#ifdef FOR_DUAL_BAND
	set_lock_down_stat_one_interface(pCtx, lock, pinReach, "wlan1");
#endif

	/* update apmib setting */
	apmib_update(CURRENT_SETTING);
#else	// LINUX SDK
	/* now 2g and 5g share the same one wsc daemon, then flash mib should be sync together.
	    If there are 2 wsc daemon under AP mode, then flash mib should be changed based on
	    each wlan interface */
	write_param_to_flash_for_tr069(GET_CURRENT_INTERFACE, (void *)&inValue, FLAG_LOCK_DOWN);
	if(strcmp(GET_CURRENT_INTERFACE, "wlan0")==0) {
#ifdef FOR_DUAL_BAND		// sync configured state
		if(pCtx->is_ap && !pCtx->wlan1_wsc_disabled) {
			if(!pCtx->ProfileDontBothApply) {
				UTIL_DEBUG("lock down state apply to both band\n");
				write_param_to_flash_for_tr069("wlan1", (void *)&inValue, FLAG_LOCK_DOWN);
			}
		}
#endif
	} else if(strcmp(GET_CURRENT_INTERFACE, "wlan1")==0) {
		if(pCtx->is_ap && !pCtx->wlan0_wsc_disabled) {
			if(!pCtx->ProfileDontBothApply) {
				UTIL_DEBUG("lock down state apply to both band\n");
				write_param_to_flash_for_tr069("wlan0", (void *)&inValue, FLAG_LOCK_DOWN);
			}
		}
	}
#endif
...
int set_last_config_err(CTX_Tp pCtx, unsigned int errFlag)
{
	unsigned int setValue = 0;
	if( !(errFlag>=CONFIG_ERR_NO_ERR && errFlag<=CONFIG_ERR_DEV_PASS_AUTH_FAIL) ) {
		UTIL_DEBUG("invalid err flag\n");
		return -1;
	}
	setValue = errFlag;
	UTIL_DEBUG("err code: %d\n", errFlag);
#if defined(__ECOS) //&& defined(HAVE_APMIB)
	set_last_config_err_one_interface(pCtx, errFlag, GET_CURRENT_INTERFACE);
#if 0//def FOR_DUAL_BAND
	char *ifname_next = NULL;
	ifname_next = get_next_wlan_interface(pCtx);
	if( ifname_next!= NULL )
		set_last_config_err_one_interface(pCtx,errFlag,ifname_next);
#endif

	/* update mib setting */
	apmib_update(CURRENT_SETTING);
#else // LINUX SDK	
	write_param_to_flash_for_tr069(GET_CURRENT_INTERFACE, (void *)&setValue, FLAG_ERRCODE);
#endif
	return 0;
}

int set_external_registrar_info(CTX_Tp pCtx, STA_CTX_Tp pSta, unsigned int flag)
{
	char uuidtmp[UUID_LEN*2+1] = {0};
	extRegInfoT extReg;
	unsigned int maxEntryNum = pCtx->ER_num;
	...
	if(flag & FLAG_ER_INCR) {
		if( maxEntryNum+1>MAX_EXTERNAL_REGISTRAR_NUM ) {
			UTIL_DEBUG("exceed max external registrar number, cannot add new!!! \n");
			return -1;
		}
	} else {
		if( maxEntryNum<=0 ) {
			UTIL_DEBUG("external registrar number less than 1, cannot delete !!! \n");
			return -1;
		}
	}

#if defined(__ECOS) //&& defined(HAVE_APMIB)
	set_external_registrar_info_one_interface(pCtx, pSta, flag, "wlan0");
#ifdef FOR_DUAL_BAND
	set_external_registrar_info_one_interface(pCtx, pSta, flag, "wlan1");
#endif
...
#else // LINUX SDK
	if(flag & FLAG_ER_DECRALL) {
		write_param_to_flash_for_tr069(GET_CURRENT_INTERFACE, NULL, flag);
		if(strcmp(GET_CURRENT_INTERFACE, "wlan0")==0) {
#ifdef FOR_DUAL_BAND		// sync configured state
			if(pCtx->is_ap && !pCtx->wlan1_wsc_disabled) {
				if(!pCtx->ProfileDontBothApply) {
					UTIL_DEBUG("ER information apply to both band\n");
					write_param_to_flash_for_tr069("wlan1", NULL, flag);
				}
			}
#endif
		} else if(strcmp(GET_CURRENT_INTERFACE, "wlan1")==0) {
			if(pCtx->is_ap && !pCtx->wlan0_wsc_disabled) {
				if(!pCtx->ProfileDontBothApply) {
					UTIL_DEBUG("ER information apply to both band\n");
					write_param_to_flash_for_tr069("wlan0", NULL, flag);
				}
			}
		}
	} else {
		memset(&extReg, 0, sizeof(extReg));
		convert_bin_to_str(pSta->uuid, 16, uuidtmp);
		//memcpy(extReg.uuid, uuidtmp, UUID_LEN*2); // ori
		//memcpy(extReg.devicename, pSta->devname, strlen((char *)pSta->devname));
		snprintf(extReg.uuid, UUID_LEN*2+1, "%s" , uuidtmp);
		snprintf(extReg.devicename, MAX_DEVICE_NAME_LEN, "%s" , pSta->devname);
		...
		UTIL_DEBUG("uuid:%s (len:%d)\n", extReg.uuid, strlen(extReg.uuid));
		UTIL_DEBUG("devname:%s (len:%d)\n", extReg.devicename, strlen(extReg.devicename));
		write_param_to_flash_for_tr069(GET_CURRENT_INTERFACE, (void *)&extReg, flag);
		if(strcmp(GET_CURRENT_INTERFACE, "wlan0")==0) {
#ifdef FOR_DUAL_BAND		// sync configured state
			if(pCtx->is_ap && !pCtx->wlan1_wsc_disabled) {
				if(!pCtx->ProfileDontBothApply) {
					UTIL_DEBUG("ER information apply to both band\n");
					write_param_to_flash_for_tr069("wlan1", (void *)&extReg, flag);
				}
			}
#endif
		} else if(strcmp(GET_CURRENT_INTERFACE, "wlan1")==0) {
			if(pCtx->is_ap && !pCtx->wlan0_wsc_disabled) {
				if(!pCtx->ProfileDontBothApply) {
					UTIL_DEBUG("ER information apply to both band\n");
					write_param_to_flash_for_tr069("wlan0", (void *)&extReg, flag);
				}
			}
		}
	}
