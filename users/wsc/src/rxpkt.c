...
static int decrypt_setting(CTX_Tp pCtx, STA_CTX_Tp pSta, unsigned char *pMsg, int msg_len)
{
	...
	while(1) {
		...
		if(PreCred==NULL) {
			...
			if(pCtx->negoAPConfigStat == CONFIG_STATE_CONFIGURED){
				RX_DEBUG("AP = CONFIGURED\n");
			}else{
				RX_DEBUG("AP = un-CONFIGURED\n");
			}

#ifndef CONFIG_MULTI_AP
			...
			if(EnptyTypeHi==0){
				...
				RX_DEBUG("multi-cred, this cred replace previous one (higher encry)\n");	
			}
			...
			if(debug_on){
				_DEBUG_PRINT("	Auth :0x%02X ; ",ntohs(AuthType));
				_DEBUG_PRINT("OPEN=1,WPAPSK=2,SHARED=4,WPA2PSK=0x20,MIXED=0x22,WPA3SAE=0x40\n");	
			}
...
static int msgHandler_M1(CTX_Tp pCtx, STA_CTX_Tp pSta, unsigned char *pMsg, int msg_len)
{
	...
	pSta->backhaul_sta = 0;
	pData = check_tag(pCtx, pMsg, msg_len, TAG_VENDOR_EXT, 0, "VendorExt", TYPE_BYTE, &tag_len);
	unsigned char sub_id;
	unsigned sub_len; 
	if( 0x00 != pData[0] || 0x37 != pData[1] || 0x2A != pData[2] ) {
		return -1;
	}
	int i = 3;
	while( i < tag_len ) {
