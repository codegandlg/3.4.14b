...
static int add_credential_attr(CTX_Tp pCtx, STA_CTX_Tp pSta, unsigned char *out, int include_cred)
{
	...
	TX_DEBUG("AuthType=%d pSta->auth_type_flags=%d pCtx->auth_type_flash=%d\n",AuthType,pSta->auth_type_flags,pCtx->auth_type_flash);
	if (AuthType) {
		if(AuthType & WSC_AUTH_WPA3SAE) {
			AuthType = WSC_AUTH_WPA3SAE;
		} else if (AuthType & WSC_AUTH_WPA2PSK) {
...
static int add_credential_attr2(CTX_Tp pCtx, STA_CTX_Tp pSta, unsigned char *out, int include_cred)
{
	...
	TX_DEBUG("AuthType=%d pSta->auth_type_flags=%d pCtx->auth_type_flash2=%d\n",AuthType,pSta->auth_type_flags,pCtx->auth_type_flash2);
	if (AuthType) {
		if(AuthType & WSC_AUTH_WPA3SAE) {
			AuthType = WSC_AUTH_WPA3SAE;
		} else if (AuthType & WSC_AUTH_WPA2PSK) {
	...
	if (EncryType == WSC_ENCRYPT_WEP && pCtx->wep_transmit_key2) {
		...
		convert_char_to_str(pCtx->network_key2,pCtx->network_key_len2,tmp1);
...
static int add_backhaulcredential_attr(CTX_Tp pCtx, STA_CTX_Tp pSta, unsigned char *out)
{
	...
	if (COME_FROM_WLAN0 == pCtx->InterFaceComeIn) {
		byteVal             = 0;
		char *interfaces[5] = { "wlan0", "wlan0-va0", "wlan0-va1", "wlan0-va2", "wlan0-va3" };

		for (i = 0; i < 5; i++) {
			wlioctl_get_mib_byte_map(interfaces[i], "multiap_bss_type", &byteVal, 1);
			if (byteVal & 0x40) {
				backhaul_interface = interfaces[i];
				break;
			}
		}
	} else if (COME_FROM_WLAN1 == pCtx->InterFaceComeIn) {
		byteVal             = 0;
		char *interfaces[5] = { "wlan1", "wlan1-va0", "wlan1-va1", "wlan1-va2", "wlan1-va3" };
	...
	wlioctl_get_mib_byte_map(backhaul_interface, "ssid", tmpbuf, sizeof(tmpbuf));

	pMsg = add_tlv(pMsg, TAG_SSID, strlen((char *)tmpbuf), tmpbuf);
	TX_DEBUG("Backhual SSID[%s]\n", tmpbuf);

	wlioctl_get_mib_byte_map(backhaul_interface, "psk_enable", tmpbuf, sizeof(int) / sizeof(char));
#ifdef L_ENDIAN
	AuthType = tmpbuf[0];
#else
	AuthType = tmpbuf[sizeof(int) / sizeof(char) - 1];
#endif

	if (0 == (0x02 & AuthType)) {
		TX_DEBUG("AuthType ERROR psk_enable: %d\n", AuthType);
		// Multi AP Backhaul should be WPA2-Personal
		return -1;
	}
	...
	wlioctl_get_mib_byte_map(backhaul_interface, "passphrase", tmpbuf, sizeof(tmpbuf));
	pMsg = add_tlv(pMsg, TAG_NETWORK_KEY, strlen((char *)tmpbuf), tmpbuf);
	TX_DEBUG("network_key[%s]\n", (char *)tmpbuf);

	// get_mac_address_map(backhaul_interface, tmpbuf);
...
int send_frag_msg(CTX_Tp pCtx, STA_CTX_Tp pSta ,  int NextStat , int times )
{
	...
	unsigned short origTotalMsgLen = 0;
...
#ifdef CONFIG_MULTI_AP
	if(pSta->backhaul_sta && pCtx->is_ap){
		size = add_backhaulcredential_attr(pCtx, pSta, tmpbuf);
	} else {
	#ifdef FOR_DUAL_BAND
		if(pCtx->both_band_credential) {
...
//#elif defined(CONFIG_IWPRIV_INTF)
	...
	if (id >= 0) {
		...
		size = BN_bn2bin(a->pub_key, tmpbuf);
		if (size < PUBLIC_KEY_LEN)
			add_prefix_padding(tmpbuf, size);
...
int send_wsc_M1(CTX_Tp pCtx, STA_CTX_Tp pSta)
{
	...
	char *        map_interface_name = NULL;
	if (COME_FROM_WLAN0 == pCtx->InterFaceComeIn) {
		map_interface_name = pCtx->wlan_interface_name;

	} else if (COME_FROM_WLAN1 == pCtx->InterFaceComeIn) {
		map_interface_name = pCtx->wlan_interface_name2;
	}
	wlioctl_get_mib_byte_map(map_interface_name, "multiap_bss_type", &multiap_bss_type, 1);
	TX_DEBUG("[WSC] [%s %d] %s mib = %02x\n", __FUNCTION__, __LINE__, map_interface_name, multiap_bss_type);
	if (multiap_bss_type & 0x80) {
		unsigned char vendor_content[9];
		memcpy(vendor_content, WSC_VENDOR_V2, 6);
		vendor_content[6] = 0x06;
		vendor_content[7] = 0x01;
		vendor_content[8] = 0x80;
		pMsg              = add_tlv(pMsg, TAG_VENDOR_EXT, 9, (void *)vendor_content);
	} else {
		pMsg = add_tlv(pMsg, TAG_VENDOR_EXT, 6, (void *)WSC_VENDOR_V2);
	}
	#else
	...
	wlioctl_get_mib_byte_map(map_interface_name, "multiap_bss_type", &multiap_bss_type, 1);
	TX_DEBUG("[WSC] [%s %d] mib = %02x\n", __FUNCTION__, __LINE__, multiap_bss_type);
	if (multiap_bss_type & 0x80) {
		pMsg = add_tlv(pMsg, TAG_VENDOR_EXT, 6, (void *)MULTIAP_BACKHAUL_STA);
	}
#endif
