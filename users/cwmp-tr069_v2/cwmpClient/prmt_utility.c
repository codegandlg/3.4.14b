...
//printf("total_len=%d\n",total_len);
	do {
		...
		if( memcmp(pHeader->signature, CURRENT_SETTING_HEADER_TAG, TAG_LEN) &&
			...
			memcmp(pHeader->signature, HW_SETTING_HEADER_TAG, TAG_LEN) &&
		...
		{
			//mismatch all supported configuration header
			CWMPDBG_FUNC(MODULE_CORE, LEVEL_ERROR, ("<%s:%d>current pHeader %02X%02X%02X%02X mismatch all supported header! Exit upgrade flow.\n",
				__FUNCTION__,__LINE__, pHeader->signature[0], pHeader->signature[1], pHeader->signature[2], pHeader->signature[3]));
